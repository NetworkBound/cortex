...
export async function getSessions(limit = 50): Promise<SessionSummary[]> {
  return jsonOrThrow<SessionSummary[]>(
    await fetch(`/api/sessions?limit=${encodeURIComponent(limit)}`),
  );
}

export async function getSessionMessages(id: string): Promise<StoredMessage[]> {
  return jsonOrThrow<StoredMessage[]>(
...
/** Import a raw chat-history export (Claude.ai / ChatGPT / generic JSON) by
 *  posting its text content. `format` defaults to "auto" (server sniffs it). */
export async function importChatFile(
  content: string,
  format: ImportFormat = "auto",
): Promise<ImportResult> {
  return jsonOrThrow<ImportResult>(
 ...
 *  The token is sent once and never persisted/logged client-side. Unofficial
 *  endpoints — may 502 or break without notice. */
export async function importChatPull(
  provider: ImportProvider,
  token: string,
): Promise<ImportResult> {
  return jsonOrThrow<ImportResult>(
    await fetch("/api/import/pull", {
      method: "POST",
      headers: { "content-type": "application/json" },
