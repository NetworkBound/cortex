...
mod tests {
    ...
    /// Build a HookSpec that runs a tiny shell script portably. Callers pass
    /// per-platform scripts because sh and cmd.exe share almost no syntax.
    fn shell_spec(unix_script: &str, win_script: &str, timeout_ms: u64) -> HookSpec {
        if cfg!(windows) {
            HookSpec {
                command: "cmd".into(),
                args: vec!["/C".into(), win_script.into()],
    ...
    async fn missing_command_does_not_block() {
        let missing = if cfg!(windows) {
            r"C:/definitely/not/a/real/binary/cortex_hook_test"
        } else {
            "/definitely/not/a/real/binary/cortex_hook_test"
        };
        let spec = HookSpec {
    ...
    async fn exit_zero_is_allow() {
        ...
        let spec = shell_spec("exit 0", "exit 0", 2000);
        let r = run_hook(&spec, "PreToolUse", "{}").await;
        assert!(!r.block);
    ...
    async fn exit_two_blocks() {
        let spec = shell_spec("echo nope 1>&2; exit 2", "echo nope 1>&2 & exit 2", 2000);
        let r = run_hook(&spec, "PreToolUse", "{}").await;
        assert!(r.block);
    ...
    async fn timeout_does_not_deadlock() {
        let spec = shell_spec("sleep 5", "ping -n 6 127.0.0.1 > nul", 150);
