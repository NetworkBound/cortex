...
pub(crate) fn default_obsidian_vault() -> Option<PathBuf> {
    ...
    // 3. WSL: explicit cross-mount probe so dev builds running from WSL
    //    still find a Windows-side vault. The username is derived from the
    //    mounted user dirs rather than hardcoded.
    if let Ok(users) = std::fs::read_dir("/mnt/c/Users") {
        for user_dir in users.flatten().map(|e| e.path()) {
            for sub in ["Documents/Cortex Brain", "Documents/Obsidian"] {
                let p = user_dir.join(sub);
                if p.is_dir() {
                    return Some(p);
                }
            }
