...
async fn gather_specs(base: &str) -> HostSpecs {
    ...
    // stall for seconds on a wedged driver.
    let ((ram_total_mb, ram_avail_mb), (gpu_name, vram_total_mb, has_cuda)) =
        tokio::task::spawn_blocking(|| (detect_ram_mb(), detect_gpu()))
            .await
            .unwrap_or(((0, 0), (None, None, false)));
    HostSpecs {
        cpu_cores,
        ram_total_mb,
        ram_avail_mb,
