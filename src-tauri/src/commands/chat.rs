...
/// One in-flight local (non-gateway) agent run, registered under the
/// synthetic `local-<ulid>` run id injected into its `Started` event.
struct LocalRun {
    abort: tokio::task::AbortHandle,
    /// Set by [`abort_local_run`] before aborting, so the event loop can
    /// report "stopped by user" instead of a generic channel-closed error.
    stopped_by_user: Arc<AtomicBool>,
}

/// In-flight local adapter runs (Ollama, CLI agents, direct APIs…). The
/// gateway's `stop_run` can only stop runs the gateway started; these run as
/// plain tokio tasks in-process, so Stop must abort them here. Entries are
/// removed on natural completion (event-loop end) or by [`abort_local_run`].
static LOCAL_RUNS: Lazy<parking_lot::Mutex<HashMap<String, LocalRun>>> =
    Lazy::new(|| parking_lot::Mutex::new(HashMap::new()));

/// Abort a local run by its synthetic run id. Returns false when the id is
/// unknown (e.g. a gateway run id) so the caller can fall through.
pub(crate) fn abort_local_run(run_id: &str) -> bool {
    let map = LOCAL_RUNS.lock();
...
/// In-process pending approvals, keyed by their synthetic run id
/// (`mcp-approval-<ulid>`). The gateway resolves its own approvals via
/// `POST approve_run`; these belong to *local* waits — an MCP chat tool call
/// paused on an 'ask'-trust server (issue 008) — so `approve_run` (and the
...
pub(crate) fn register_local_approval(run_id: String) -> tokio::sync::oneshot::Receiver<String> {
    let (tx, rx) = tokio::sync::oneshot::channel();
    LOCAL_APPROVALS.lock().insert(run_id, tx);
    rx
}

/// Resolve a pending in-process approval with the user's choice. Returns
/// false when the id is unknown (e.g. a gateway run id) so the caller can
/// fall through to the gateway.
pub(crate) fn resolve_local_approval(run_id: &str, choice: &str) -> bool {
    let sender = LOCAL_APPROVALS.lock().remove(run_id);
    match sender {
        // A send failure means the waiter already gave up (timeout/abort);
        // the entry is gone either way, so still report "handled locally".
        Some(sender) => {
...
///
/// MCP-origin tools (issue 008) carry a qualified `mcp__<server>__<tool>`
/// name whose prefix would defeat the read/write token classification (and
/// whose capability lives inside the server process, not the filesystem), so
/// they route through `tier_allows_mcp` on the bare tool name. Everything
/// else keeps the exact pre-existing `tier_allows` behavior.
...
/// Safe Mode (issue 004) tier clamp: `DangerFullAccess → WorkspaceWrite`, or
/// — full scope, the built-in "CI-safe" profile's `force_read_only` flag —
/// all the way to `ReadOnly`. Narrow-only both ways: the plain clamp only
/// tightens the single widest tier, and `force_read_only` can only tighten
/// further (`ReadOnly` is already the strictest tier, so clamping any other
/// tier down to it never widens anything).
fn clamp_tier_for_safe_mode(tier: SandboxTier, force_read_only: bool) -> SandboxTier {
    if force_read_only {
        return SandboxTier::ReadOnly;
    }
    match tier {
        SandboxTier::DangerFullAccess => SandboxTier::WorkspaceWrite,
        other => other,
    }
}

/// Safe Mode approval-policy clamp: `Never → Untrusted` (auto-approval of
/// write/exec is off; provably read-only inspection may still skip the
/// prompt). Narrow-only: `OnRequest` (asks for *everything*, and the pin
/// chat_send applies to untrusted projects) is stricter than `Untrusted`
/// and must NOT be loosened, so it passes through unchanged.
fn clamp_policy_for_safe_mode(policy: ApprovalPolicy) -> ApprovalPolicy {
    match policy {
...
/// Safe Mode command-policy gate. Slots BETWEEN the tier gate and the
/// guardrails in the ToolCall chain: a policy `Deny` short-circuits exactly
/// like a tier rejection, BEFORE any approval UI can see the call. `None`
/// when Safe Mode is off (`policy` is `None` — absent files, zero behavior
/// change) or when the policy has no Deny for this call. Narrow-only: an
/// `Allow`/`Ask` here changes nothing in this chain — the guardrails and
/// plan-mode gate still run after it (see the matching invariant note in
/// `approval_policy.rs`).
fn maybe_block_by_command_policy(
    policy: Option<&CommandPolicy>,
    name: &str,
    args_json: &str,
) -> Option<String> {
    let decision = policy?.evaluate_tool_call(name, args_json)?;
    ...
    Some(format!(
        "safe mode: tool '{name}' blocked by command policy rule '{rule}' ({source}) — {reason}",
        source = decision.source
    ))
}

/// Safe Mode auto-approve suppressor: a shell command the policy does not
/// explicitly Allow (a Deny, an Ask, an allowlist-mode miss, or an exec tool
/// with no extractable command) can never be silently auto-approved — the
/// request is forwarded to the user instead. Returns the reason, or `None`
/// to leave the auto-approve decision untouched (Safe Mode off, non-exec
/// tool, or an explicit Allow).
fn command_policy_suppresses_auto_approve(
    policy: Option<&CommandPolicy>,
    tool_name: &str,
    payload_json: &str,
) -> Option<String> {
    let decision = policy?.evaluate_tool_call(tool_name, payload_json)?;
    match decision.action {
        PolicyAction::Allow => None,
        PolicyAction::Deny | PolicyAction::Ask => Some(format!(
            "command policy says {:?} (rule: {}, source: {})",
            decision.action,
            decision.matched.as_deref().unwrap_or("<default>"),
            decision.source
        )),
    }
}

/// Resolve the trust-matrix auto-approve source for a `chat_send` call:
/// `None` for an untrusted project, else the on-disk matrix (missing/corrupt
/// file → `TrustMatrix::default()`, which is all-off — see
/// `TrustMatrix::load`). Mirrors `approval_policy` being pinned to
/// `OnRequest` for an untrusted project just above this call site — a
/// user's global "auto-approve all commands" toggle must never silently
/// apply to a repo they haven't reviewed yet.
///
/// Extracted as a pure function (rather than left as the inline ternary it
...
/// Whether an `ApprovalRequest`'s tool name may EVER be resolved by the
/// generic auto-approve mechanisms (the allowlist, the project approval
/// policy, or the trust matrix) — `false` for any MCP-originated request
/// (issue 008, qualified `mcp__<server>__<tool>` names).
///
/// `run_gated_mcp_call` only ever registers a local approval — the thing that
/// produces one of these `ApprovalRequest`s — for a server whose trust is
/// `ask`; `trusted` servers execute without one, and `untrusted` servers are
/// refused before one is ever created. So by construction, any MCP approval
/// request reaching this point represents a per-server, deliberately opted-
/// into "always confirm this one" decision. The generic mechanisms here
/// predate MCP and reason only about tier/guardrails-shaped tool names
/// (`write_file`, `shell_exec`, …); letting a broad allowlist glob, a
/// project's `never` approval policy, or the trust matrix silently satisfy an
/// MCP `ask` would make that trust level meaningless — the exact bypass
/// Agent E flagged for issue 008.
fn eligible_for_generic_auto_approve(tool_name: &str) -> bool {
    !tool_name.starts_with(crate::mcp::chat_tools::MCP_TOOL_PREFIX)
}

// ---------------------------------------------------------------------------
// Issue 008 — model-initiated MCP tool calls (the chat dispatcher)
// ---------------------------------------------------------------------------

/// How long an 'ask'-trust MCP tool call waits for the user's decision before
/// failing closed.
const MCP_APPROVAL_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(300);

/// Cap on the tool-result text mirrored into the `ToolResult` event (the full
/// result still returns to the model). Keeps trace rows bounded.
const MCP_RESULT_EVENT_CAP: usize = 2000;

/// Gate context for one model-initiated MCP tool call, resolved exactly the
/// way `chat_send` resolves its per-turn gates: trust-gated guardrails +
/// sandbox tier (untrusted/no project → ReadOnly via `effective_tier`), Safe
/// Mode clamps + command policy, and the current plan/act mode.
pub(crate) struct McpChatGateCtx {
    pub mode: String,
...
impl McpChatGateCtx {
    pub(crate) fn resolve(project_root: Option<&Path>) -> Self {
        let is_trusted = project_root.map(trust::is_trusted).unwrap_or(false);
        let guardrails = if is_trusted {
            project_root
                .map(Guardrails::load)
        ...
        let tier = orchestrator::effective_tier(project_root);
        let safe_mode_state = crate::commands::safe_mode::load();
        let safe_mode_on = safe_mode_state.enabled;
        let tier = if safe_mode_on {
            clamp_tier_for_safe_mode(tier, safe_mode_state.force_read_only)
        } else {
            tier
        };
        let command_policy = if safe_mode_on {
            Some(orchestrator::command_policy::load_effective(project_root))
        } else {
            None
        };
        Self {
...
/// The chat gate chain for an MCP tool call, mirroring the event loop's
/// ToolCall order exactly: sandbox tier (deny-bias, MCP-aware) → Safe Mode
/// command policy → guardrails/plan-mode. Runs on the server-local (bare)
/// tool name so plan-mode's `^write|^edit|…` anchor and the tier's read/write
...
fn gate_mcp_chat_call(ctx: &McpChatGateCtx, bare_tool: &str, args_json: &str) -> Option<String> {
    ...
    maybe_block_by_command_policy(ctx.command_policy.as_ref(), bare_tool, args_json)
        .or_else(|| maybe_block_tool_call(&ctx.mode, bare_tool, args_json, &ctx.guardrails))
}

/// Execute one model-initiated MCP tool call from a chat turn (issue 008).
///
/// This is the ONLY path from an adapter to an MCP server, and every call
/// runs the full pipeline before executing: per-server chat exposure +
/// per-tool enablement, the sandbox-tier/Safe-Mode/guardrail chain
/// (`gate_mcp_chat_call`), then the per-server trust tier — `trusted` runs,
/// `ask` pauses on an in-process `ApprovalRequest` the user resolves through
/// the normal approval UI, `untrusted` refuses. Execution itself goes through
/// `commands::mcp::call_tool_gated`, the same audited choke-point the manual
/// panel button uses. Emits a paired `ToolCall`/`ToolResult` on every path so
/// Run Replay captures allowed, denied, and failed attempts alike.
pub(crate) async fn dispatch_mcp_chat_tool(
    project_root: Option<&Path>,
    qualified: &str,
    args: &serde_json::Value,
    tx: &mpsc::Sender<AgentEvent>,
) -> Result<String, String> {
    let servers = crate::mcp::config::load();
    let ctx = McpChatGateCtx::resolve(project_root);
    dispatch_mcp_chat_tool_with(&servers, &ctx, qualified, args, tx).await
}

/// Core of [`dispatch_mcp_chat_tool`] with the registry snapshot and gate
/// context injected, so the pipeline is unit-testable without global state.
pub(crate) async fn dispatch_mcp_chat_tool_with(
    servers: &[crate::mcp::config::McpServerConfig],
    ctx: &McpChatGateCtx,
    qualified: &str,
    args: &serde_json::Value,
    tx: &mpsc::Sender<AgentEvent>,
) -> Result<String, String> {
    let Some((cfg, tool)) = crate::mcp::chat_tools::resolve_in(servers, qualified) else {
        return Err(format!(
            "unknown MCP tool '{qualified}' (no configured server matches)"
        ));
    };
    let args_json = args.to_string();
    let started = std::time::Instant::now();

    // Surface the attempt immediately: Run Replay records the ToolCall, the
    // PreToolUse hook fires, and the event loop's display gates annotate it.
    let _ = tx
        .send(AgentEvent::ToolCall {
            name: qualified.to_string(),
            args: args.clone(),
            preview: Some(format!("MCP · {} · {tool}", cfg.name)),
        })
        .await;

    let outcome = run_gated_mcp_call(&cfg, &tool, qualified, args, &args_json, ctx, tx).await;

    let (ok, text) = match &outcome {
        Ok(result) => (true, result.as_str()),
        Err(reason) => (false, reason.as_str()),
    };
    let summary: String = text.chars().take(MCP_RESULT_EVENT_CAP).collect();
    let _ = tx
        .send(AgentEvent::ToolResult {
            name: qualified.to_string(),
            ok,
            // Tool output is free text that can embed a secret — pass the
            // event copy through the redact:: choke-point before it reaches
            // the UI or the trace store. The un-redacted result still returns
            // to the model, which needs the real content to keep working.
            summary: crate::redact::redact_text(&summary),
...
/// Gate chain + trust approval + execution for one MCP chat tool call. Split
/// from the dispatcher so the ToolCall/ToolResult event pair around it stays
/// paired on every path (allowed, denied, failed).
async fn run_gated_mcp_call(
    ...
    // Defense in depth: adapters only advertise exposed servers, but a model
    // can name any tool it likes — re-check exposure and per-tool enablement
    // here (and `call_tool_gated` re-checks enablement + trust once more).
    if !cfg.expose_in_chat {
        return Err(format!(
            "server '{}' does not expose its tools to chat (enable it in the MCP panel)",
            cfg.name
        ));
    }
    if !crate::mcp::config::tool_enabled(cfg, tool) {
        return Err(format!(
            "tool '{tool}' is disabled for server '{}'",
            cfg.name
        ));
    }
    if let Some(reason) = gate_mcp_chat_call(ctx, tool, args_json) {
        return Err(reason);
    }

    // Per-server trust tier (issue 009): trusted runs, ask pauses for an
    // explicit user decision, untrusted refuses.
    match cfg.trust {
        crate::mcp::config::McpTrustLevel::Untrusted => {
            return Err(format!(
                "server '{}' is marked untrusted: tool calls are refused",
                cfg.name
            ));
        }
        crate::mcp::config::McpTrustLevel::Trusted => {}
        crate::mcp::config::McpTrustLevel::Ask => {
            let run_id = format!("mcp-approval-{}", ulid::Ulid::new());
            let rx = register_local_approval(run_id.clone());
            let args_preview: String = args_json.chars().take(500).collect();
            let _ = tx
                .send(AgentEvent::ApprovalRequest {
                    run_id: run_id.clone(),
                    tool: Some(qualified.to_string()),
                    preview: Some(format!(
                        "MCP tool '{tool}' on server '{}' (trust: ask)\nargs: {args_preview}",
                        cfg.name
                    )),
                    choices: vec!["once".to_string(), "deny".to_string()],
                ...
                .await;
            let choice = match tokio::time::timeout(MCP_APPROVAL_TIMEOUT, rx).await {
                Ok(Ok(choice)) => choice,
                Ok(Err(_)) => {
                    return Err(format!(
                        "approval for MCP tool '{tool}' was dropped before a decision"
                    ));
                }
                Err(_) => {
                    // Timed out — clear the stale entry so a very late click
                    // in the UI can't approve a call that already failed.
                    LOCAL_APPROVALS.lock().remove(&run_id);
                    return Err(format!(
                        "approval for MCP tool '{tool}' timed out (no decision within {}s)",
                        MCP_APPROVAL_TIMEOUT.as_secs()
                    ));
                }
            };
            if choice.eq_ignore_ascii_case("deny") {
                return Err(format!("user denied MCP tool '{tool}'"));
            }
        }
    }

    // Single audited choke-point (shared with the manual panel button).
    // `approved: true` is only reachable here after the trust match above —
    // trusted, or an explicit per-call user approval for 'ask'. `ctx`'s
    // command policy (already resolved once per turn by
    // `McpChatGateCtx::resolve` — `None` when Safe Mode is off) is threaded
    // straight through so the chat path and the manual panel button consult
    // the exact same Safe Mode gate inside `call_tool_gated` (issue 009 full
    // scope) rather than each loading/duplicating it independently.
    crate::commands::mcp::call_tool_gated(
        cfg,
        tool,
        Some(args.clone()),
        true,
        crate::mcp::chat_tools::audit_store(),
        ctx.command_policy.as_ref(),
    )
    .await
}
...
pub async fn chat_send(
    ...
    // Safe Mode (issue 004): one-switch lockdown, DEFAULT-OFF. Re-read per
    // chat_send (like `AutoApproveList::load` below) so toggling takes effect
    // on the next turn without a restart. When ON, every adjustment is
    // narrow-only: tier `DangerFullAccess → WorkspaceWrite` (or, under the
    // built-in "CI-safe" profile's `force_read_only`, all the way to
    // `ReadOnly`), approval policy `Never → Untrusted`, plus the command
    // allow/denylist loaded here and enforced in the event loop between the
    // tier gate and the guardrails. Absent files ⇒ `safe_mode_on == false` ⇒
    // everything below is `None`/unchanged and behavior is bit-identical to
    // before this feature.
    let safe_mode_state = crate::commands::safe_mode::load();
    let safe_mode_on = safe_mode_state.enabled;
    let (sandbox_tier, approval_policy) = if safe_mode_on {
        (
            clamp_tier_for_safe_mode(sandbox_tier, safe_mode_state.force_read_only),
            clamp_policy_for_safe_mode(approval_policy),
        )
    } else {
        (sandbox_tier, approval_policy)
    };
    let command_policy: Option<std::sync::Arc<CommandPolicy>> = if safe_mode_on {
        Some(std::sync::Arc::new(
            orchestrator::command_policy::load_effective(project_root.as_deref()),
        ))
    } else {
        None
    };
    ...
    // Bare request: no explicit agent pick, no model. The only shape either
    // the outcome-routing hint or a session budget cap can ever apply to —
    // both gate on this same condition, mirroring the default branch's own
    // re-check in `orchestrator::route_with_outcome`.
    let is_bare_request =
        args.agent.is_none() && req.model.as_deref().map_or(true, |m| m.trim().is_empty());

    // Budget ceilings per session (issue 006 full scope). OPT-IN: a session
    // with no cap set (`session_budget_cap` returns `None`, the default) is
    // untouched — this whole block is a no-op. Enforced here, before routing
    // even runs, rather than inside `orchestrator::route`: a spend cap is a
    // guard on whether to send at all, not a routing preference, and must
    // never have a chance to touch an explicit pick or a model-carrying
    // request (both skip this branch via `is_bare_request`).
    ...
    let (decision, picked, reason, agents, planner_adapter) = {
        ...
        // Cost-per-success routing hint (issue 006). OPT-IN and DEFAULT-OFF:
        // `~/.cortex/outcome-routing.json` absent/off ⇒ `None` ⇒
        // `route_with_outcome` is byte-identical to `route`. Computed only for
        // a bare request (no explicit agent, no model) because those are the
        // only requests the hint can ever apply to — the default branch is the
        // sole consumer, and it re-checks both conditions. Thin/stale
        // reliability data ⇒ `None` ⇒ today's exact behavior. When this
                ...
                } = &evt
                {
                    ...
                    let choice = choice.filter(|_| {
                        match command_policy_suppresses_auto_approve(
                            ...
                            Some(reason) => {
                                tracing::info!(
                                    "auto-approve suppressed by Safe Mode for run {run_id} tool '{tool_name}': {reason} — forwarding approval request to user"
                                );
                                false
                            }
                        }
...
pub async fn approve_run(args: ApproveRunArgs, state: State<'_, AppState>) -> Result<(), String> {
    // Local (in-process) approvals — an MCP chat tool call waiting on an
    // 'ask'-trust decision (issue 008) — are resolved directly: the gateway
    // never saw their synthetic run ids, so POSTing there could only 404.
    // Mirrors `stop_run`'s local-first check below.
    if resolve_local_approval(&args.run_id, &args.choice) {
        return Ok(());
    }
...
mod tests {
    ...
    // --- Issue 008: MCP chat-tool dispatcher --------------------------------

    fn mcp_cfg(
        id: &str,
        trust: crate::mcp::config::McpTrustLevel,
    ...
    fn test_ctx(tier: SandboxTier, mode: &str) -> McpChatGateCtx {
        McpChatGateCtx {
            mode: mode.to_string(),
            tier,
    ...
    #[tokio::test]
    async fn local_approval_register_resolve_round_trip() {
        assert!(
            !resolve_local_approval("mcp-approval-unknown", "once"),
            "unknown id → false (falls through to the gateway)"
        );
        let rx = register_local_approval("mcp-approval-test-rt".to_string());
        assert!(resolve_local_approval("mcp-approval-test-rt", "once"));
        assert_eq!(rx.await.unwrap(), "once");
        // Resolved entries are removed — a second resolve finds nothing.
        assert!(!resolve_local_approval("mcp-approval-test-rt", "once"));
    }

    /// Approval-gate test (issue 008 test plan): an 'ask'-trust server pauses
    /// on an ApprovalRequest, and a user denial blocks execution — the paired
    ...
    async fn mcp_dispatch_ask_trust_denial_blocks_execution() {
        ...
        let handle = tokio::spawn(async move {
            dispatch_mcp_chat_tool_with(
                &servers,
                &ctx,
                "mcp__test-mcp-ask-deny__echo",
        ...
        // ToolCall first, then the approval pause. Deny it like the UI would.
        let run_id = loop {
            ...
            match evt {
                ...
                AgentEvent::ToolCall { name, .. } => {
                    assert_eq!(name, "mcp__test-mcp-ask-deny__echo");
                }
                other => panic!("unexpected event before approval: {other:?}"),
            }
        };
        assert!(resolve_local_approval(&run_id, "deny"));

        let out = handle.await.unwrap();
        let err = out.expect_err("denied call must not execute");
        assert!(err.contains("denied"), "reason names the denial: {err}");
        // The paired ToolResult records the refusal for Run Replay.
        let evt = rx.recv().await.expect("tool result after denial");
        let AgentEvent::ToolResult { ok, summary, .. } = evt else {
            panic!("expected ToolResult, got {evt:?}");
        };
        assert!(!ok);
        assert!(summary.contains("denied"));
    }

    /// An untrusted server refuses model-initiated calls outright — no
    /// approval prompt is even offered.
    #[tokio::test]
    async fn mcp_dispatch_refuses_untrusted_server() {
        ...
        let ctx = test_ctx(SandboxTier::DangerFullAccess, "act");
        let (tx, mut rx) = mpsc::channel(64);
        let out = dispatch_mcp_chat_tool_with(
            &servers,
            &ctx,
            "mcp__test-mcp-untrusted__echo",
            &serde_json::json!({}),
            &tx,
        )
        .await;
        let err = out.expect_err("untrusted server must refuse");
        assert!(err.contains("untrusted"), "{err}");
        drop(tx);
        let mut saw_approval = false;
        while let Some(evt) = rx.recv().await {
            if matches!(evt, AgentEvent::ApprovalRequest { .. }) {
                saw_approval = true;
            }
        }
        assert!(!saw_approval, "no approval prompt for an untrusted server");
    }

    /// The dispatcher fails closed on: unknown tools, servers not exposed to
    /// chat, per-tool disables, and a sandbox-tier rejection — all BEFORE any
    /// execution or approval prompt.
    #[tokio::test]
    async fn mcp_dispatch_fails_closed_on_gates() {
        let trusted = crate::mcp::config::McpTrustLevel::Trusted;
        let ctx = test_ctx(SandboxTier::DangerFullAccess, "act");
        let (tx, _rx) = mpsc::channel(64);
        let args = serde_json::json!({});

        // Unknown tool name → no server resolves.
        let err = dispatch_mcp_chat_tool_with(&[], &ctx, "mcp__nope__x", &args, &tx)
            .await
            .expect_err("unknown tool");
        assert!(err.contains("unknown MCP tool"), "{err}");

        // Server exists but is not exposed to chat (default-OFF invariant).
        let mut unexposed = mcp_cfg("test-mcp-unexposed", trusted);
        unexposed.expose_in_chat = false;
        let err = dispatch_mcp_chat_tool_with(
            &[unexposed],
            &ctx,
            "mcp__test-mcp-unexposed__echo",
            &args,
            &tx,
        )
        .await
        .expect_err("unexposed server");
        assert!(err.contains("does not expose"), "{err}");

        // Per-tool disable wins over everything else.
        let mut disabled = mcp_cfg("test-mcp-disabled", trusted);
        disabled.disabled_tools = vec!["echo".to_string()];
        let err = dispatch_mcp_chat_tool_with(
            &[disabled],
            &ctx,
            "mcp__test-mcp-disabled__echo",
            &args,
            &tx,
        )
        .await
        ...
        // Sandbox tier: an unclassified MCP tool is denied under ReadOnly.
        let ro_ctx = test_ctx(SandboxTier::ReadOnly, "act");
        let err = dispatch_mcp_chat_tool_with(
            &[mcp_cfg("test-mcp-tier", trusted)],
            &ro_ctx,
            "mcp__test-mcp-tier__do_thing",
            &args,
            &tx,
        )
        .await
        .expect_err("tier-blocked tool");
        assert!(err.contains("sandbox: blocked by tier"), "{err}");
    }

    /// Plan mode blocks write/exec-shaped MCP tools on their BARE name (the
    /// qualified prefix must not defeat the `^write|^edit|…` anchor), while
    /// read-shaped tools stay callable.
    #[test]
    fn mcp_gate_applies_plan_mode_to_bare_tool_name() {
        let plan_ctx = test_ctx(SandboxTier::DangerFullAccess, "plan");
        assert!(
            gate_mcp_chat_call(&plan_ctx, "write_file", "{}").is_some(),
            "plan mode must block a write-shaped MCP tool"
        );
        assert!(
            gate_mcp_chat_call(&plan_ctx, "read_file", "{}").is_none(),
            "plan mode keeps reads available"
        );
        let act_ctx = test_ctx(SandboxTier::DangerFullAccess, "act");
        assert!(gate_mcp_chat_call(&act_ctx, "write_file", "{}").is_none());
    }

    /// Event-capture test (issue 008 test plan): a trusted, exposed tool on a
    /// live (mock) MCP server executes end-to-end and emits the paired
    /// tool_call / tool_result events Run Replay consumes. Skips when `node`
    /// is unavailable (see `mcp::client::mock_server_tests`).
    #[tokio::test]
    async fn mcp_dispatch_executes_mock_tool_and_emits_events() {
        ...
        let ctx = test_ctx(SandboxTier::DangerFullAccess, "act");
        let (tx, mut rx) = mpsc::channel(64);
        let qualified = "mcp__test-mcp-chat-exec__echo";
        let out = dispatch_mcp_chat_tool_with(
            &[cfg.clone()],
            &ctx,
            qualified,
            &serde_json::json!({ "msg": "from-chat" }),
            &tx,
        )
        .await;
        crate::mcp::client::disconnect(&cfg.id).await.unwrap();

        let result = out.expect("trusted + exposed tool executes");
        assert!(result.contains("from-chat"), "mock echoed args: {result}");

        drop(tx);
        let (mut saw_call, mut saw_result) = (false, false);
        while let Some(evt) = rx.recv().await {
            match evt {
                AgentEvent::ToolCall { name, .. } => {
        ...
        assert!(saw_call, "tool_call captured");
        assert!(saw_result, "tool_result captured");
    }

    /// Approval-gate happy path: an 'ask' server + explicit user approval
    /// executes exactly once, through the same audited choke-point.
    #[tokio::test]
    async fn mcp_dispatch_ask_trust_approval_allows_execution() {
        use crate::mcp::client::mock_server_tests::{mock_cfg, node_available};
        if !node_available() {
            return;
        }
        ...
        let ctx = test_ctx(SandboxTier::DangerFullAccess, "act");
        let (tx, mut rx) = mpsc::channel(64);
        let servers = vec![cfg.clone()];
        let handle = tokio::spawn(async move {
            dispatch_mcp_chat_tool_with(
                &servers,
                &ctx,
                "mcp__test-mcp-chat-ask__echo",
                &serde_json::json!({ "q": 1 }),
                &tx,
            )
            .await
        ...
        let run_id = loop {
            let evt = rx.recv().await.expect("approval request arrives");
            if let AgentEvent::ApprovalRequest { run_id, .. } = evt {
                break run_id;
            }
        };
        assert!(resolve_local_approval(&run_id, "once"));

        let out = handle.await.unwrap();
        crate::mcp::client::disconnect(&cfg.id).await.unwrap();
        let result = out.expect("approved call executes");
        assert!(result.contains("\"q\":1"), "{result}");
    }

    /// The critical safety invariant of the trust-matrix wiring: an
    /// untrusted project must NEVER get a trust matrix to consult, full
    /// stop — regardless of what's saved on disk. A user could easily have
    /// "auto-approve all commands" toggled on globally (it's meant for
    /// projects they trust); an unfamiliar/malicious repo must not be able
    /// to ride that toggle just by being opened.
    #[test]
    fn untrusted_project_never_gets_a_trust_matrix() {
        assert!(resolve_trust_matrix_for_project(false).is_none());
    }

    /// A trusted project DOES get a matrix (defaulting to all-off if none
    /// was ever saved) — proves the loader path isn't itself gated away.
    #[test]
    fn trusted_project_gets_a_trust_matrix() {
        assert!(resolve_trust_matrix_for_project(true).is_some());
    }
    ...
    fn would_auto_approve(
        ...
        if !eligible_for_generic_auto_approve(tool) {
            return false; // MCP-originated: never eligible, regardless of policy
        }
    ...
    /// Issue 008 fix: an MCP `ask`-trust approval request must never be
    /// silently satisfied by the generic `never` approval policy, even when
    /// the tier is wide open and the policy alone would happily approve it.
    /// Only the dedicated per-server trust dispatcher may approve these.
    #[test]
    fn mcp_approval_requests_are_never_eligible_for_generic_auto_approve() {
        ...
        // Even the most permissive combination (never + DangerFullAccess,
        // which would auto-approve a built-in write outright) must not
        // auto-approve an MCP tool call.
        ...
        // The same shape, without the MCP prefix, IS auto-approved —
        // confirming the exclusion is specific to MCP names, not a general
        // regression in the `never` policy re-gate.
        assert!(would_auto_approve(
            ApprovalPolicy::Never,
            SandboxTier::DangerFullAccess,
            "act",
    ...
    fn expand_at_tokens_problems_surfaces_real_cargo_error() {
        ...
        assert!(
            out.contains("[error]")
                && (out.contains("src/main.rs") || out.contains(r"src\main.rs")),
    ...
    fn expand_at_tokens_attaches_terminal_output() {
        ...
        crate::paths::test_home::with_temp_home(|home| {
            std::fs::create_dir_all(home.join(".cortex")).unwrap();
            std::fs::write(
                home.join(".cortex").join("last-shell-output.log"),
                "cargo build\nerror[E0433]: failed to resolve\n",
            )
            .unwrap();
            result = Some(expand_at_tokens("why did this fail @terminal", None));
        });
    ...
    // --- Safe Mode (issue 004): clamps + command-policy gate wiring ---------
    //
    // The gates themselves are pure; these tests pin (a) the narrow-only
    // clamp direction, (b) Deny blocking BEFORE any approval path, and
    // (c) that Safe Mode OFF (policy `None`) is bit-identical to before —
    // the existing tier/guardrail/approval tests above run unchanged.

    #[test]
    fn safe_mode_clamps_are_narrow_only() {
        // Tier: only the widest tier is clamped, and only downward.
        assert_eq!(
            clamp_tier_for_safe_mode(SandboxTier::DangerFullAccess, false),
            SandboxTier::WorkspaceWrite
        );
        assert_eq!(
            clamp_tier_for_safe_mode(SandboxTier::WorkspaceWrite, false),
            SandboxTier::WorkspaceWrite
        );
        assert_eq!(
            clamp_tier_for_safe_mode(SandboxTier::ReadOnly, false),
            SandboxTier::ReadOnly
        );
        // CI-safe preset (issue 004 full scope): `force_read_only` clamps
        // EVERY tier down to ReadOnly — still narrow-only, since ReadOnly is
        // already the strictest tier.
        for t in [
            SandboxTier::ReadOnly,
            SandboxTier::WorkspaceWrite,
            SandboxTier::DangerFullAccess,
        ...
        // loosened to Untrusted, which would auto-approve read-only calls.
        assert_eq!(
            clamp_policy_for_safe_mode(ApprovalPolicy::Never),
            ApprovalPolicy::Untrusted
        );
        assert_eq!(
            clamp_policy_for_safe_mode(ApprovalPolicy::OnRequest),
            ApprovalPolicy::OnRequest
        );
        assert_eq!(
            clamp_policy_for_safe_mode(ApprovalPolicy::Untrusted),
            ApprovalPolicy::Untrusted
        );
    }

    /// Safe Mode OFF ⇒ the policy handle is `None` ⇒ both gates are inert,
    /// even for a command a policy would deny. Zero default-behavior drift.
    #[test]
    fn safe_mode_off_command_policy_gates_are_inert() {
        let payload = r#"{"cmd":"rm -rf /"}"#;
        assert!(maybe_block_by_command_policy(None, "shell_exec", payload).is_none());
        assert!(command_policy_suppresses_auto_approve(None, "shell_exec", payload).is_none());
    }

    #[test]
    fn command_policy_deny_blocks_the_tool_call_with_rule_and_source() {
        let p = CommandPolicy::from_files(
            Some("[[rule]]\npattern = \"rm *\"\naction = \"deny\"\nreason = \"no deletions\"\n"),
            None,
        );
        ...
        assert!(msg.contains("no deletions"), "{msg}");
    ...
    /// still blocks the same call in the gate that runs right after
    /// (narrow-only invariant: this layer never widens the guardrails).
    #[test]
    fn command_policy_allow_does_not_override_high_risk_guardrails() {
        let p = CommandPolicy::from_files(
            Some("[[rule]]\npattern = \"rm *\"\naction = \"allow\"\n"),
            None,
        );
        let payload = r#"{"cmd":"rm -rf /"}"#;
        // The policy gate itself lets it through…
        assert!(maybe_block_by_command_policy(Some(&p), "shell_exec", payload).is_none());
    ...
    #[test]
    fn command_policy_suppresses_auto_approve_unless_explicitly_allowed() {
        let raw = concat!(
            "default_ask = true\n",
            "[[rule]]\npattern = \"git status*\"\naction = \"allow\"\n",
            "[[rule]]\npattern = \"rm *\"\naction = \"deny\"\n",
        ...
        let p = CommandPolicy::from_files(Some(raw), None);
        // Explicit Allow → the auto-approve decision is untouched.
        assert!(command_policy_suppresses_auto_approve(
            Some(&p),
            "shell_exec",
            r#"{"cmd":"git status"}"#
        )
        .is_none());
        // Deny, Ask, an allowlist-mode miss, and an exec tool with no
        // extractable command all force the prompt.
        for payload in [
            r#"{"cmd":"rm -rf x"}"#,
            r#"{"cmd":"git push --force"}"#,
            r#"{"cmd":"python evil.py"}"#,
            "{}",
        ] {
            assert!(
                command_policy_suppresses_auto_approve(Some(&p), "shell_exec", payload).is_some(),
                "expected suppression for {payload}"
            );
        }
        // Non-exec tools: no opinion (the existing re-gate owns those).
        assert!(command_policy_suppresses_auto_approve(
            Some(&p),
            "write_file",
            r#"{"path":"a.txt"}"#
