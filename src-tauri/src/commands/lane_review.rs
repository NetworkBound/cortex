...
/// still moving. `pub(crate)` so the issues pipeline (007) reuses the exact
/// same eligibility rule for its approval-gated PR path.
pub(crate) fn reviewable_lane(
    store: &LaneStore,
    run_id: &str,
) -> Result<(LaneRunRecord, String), String> {
    let lane = store
        .get(run_id)
