...
/// Current cost-per-success (outcome-aware) routing toggle — issue 006.
/// DEFAULT-OFF: a missing `~/.cortex/outcome-routing.json` reads as `false`
/// and routing is byte-identical to today.
#[tauri::command]
pub async fn get_outcome_routing() -> Result<bool, String> {
    Ok(crate::orchestrator::cost_router::outcome_routing_enabled())
}

/// Toggle cost-per-success routing. Takes effect on the next message —
/// `chat_send` re-reads the flag every turn, no restart needed.
#[tauri::command]
pub async fn set_outcome_routing(enabled: bool) -> Result<bool, String> {
    crate::orchestrator::cost_router::write_outcome_routing(enabled)
        .map_err(|e| format!("write outcome-routing.json: {e}"))?;
    Ok(enabled)
}

/// Current spend cap for one session (issue 006 full scope — budget
/// ceilings). `None` — no `~/.cortex/session-budgets/<id>.json`, the default
/// — means "no cap", and `chat_send`'s routing/spend-guard behaves exactly as
/// it does today.
#[tauri::command]
