...
/// First vault key for `provider`, regardless of label — for callers that
/// key on provider alone (e.g. the issues pipeline resolving a forge PAT).
/// `Ok(None)` when the vault has no key for that provider (a missing vault
