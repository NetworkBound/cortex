...
pub async fn reindex(
    ...
    for (id, session_id, ts, role, content) in pending {
        ...
        match embed_text(ollama_base, model, &text).await {
            ...
            Ok(v) if store
                .upsert_chat_embedding(&id, &session_id, ts, &role, &content, model, &v)
                .is_ok() =>
            {
                embedded += 1;
            }
...
pub async fn reindex_notes(
    ...
    let needs_work: Vec<(PathBuf, String, i64)> = paths
        .into_iter()
        .filter_map(|p| {
    ...
    let mut embedded = 0usize;
    let mut failed = 0usize;
    for (p, path_str, mtime) in needs_work {
        ...
        match embed_text(ollama_base, model, &text).await {
            Ok(v) if store
                .upsert_chat_embedding(&path_str, "vault", mtime, "note", &content, model, &v)
                .is_ok() =>
            {
                embedded += 1;
            }
...
pub async fn search(
    ...
    let mut hits: Vec<ChatSemanticHit> = rows
        .into_iter()
        .filter(|(_, _, _, role, _, _)| include_notes || role != "note")
        .map(|(message_id, session_id, ts, role, text, vec)| ChatSemanticHit {
            score: cosine(&q, &vec),
