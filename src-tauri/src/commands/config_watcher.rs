...
const TOP_LEVEL_FILES: &[&str] = &[
    ...
    "webhooks.json",
    "themes.json",
    // Safe Mode (issue 004): the StatusBar shield badge subscribes to these
    // so a toggle/policy edit shows up without a restart. The chat gate
    // itself re-reads them per turn, independent of this event stream.
    "safe-mode.json",
    "command-policy.toml",
];
