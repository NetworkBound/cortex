...
mod tests {
    ...
    fn find_path_line_picks_first_existing_in_stack() {
        ...
        // The join uses the platform separator, so build the expectation the same way.
        let expected = std::path::Path::new("/tmp").join("nope/does-not-exist.ts");
        assert_eq!(out, Some((expected.display().to_string(), 42)));
    }
