...
/// `owner/repo` the updater pulls releases from. Resolved via
/// `CORTEX_UPDATE_REPO` env → `~/.cortex/infra.json` (`update_repo`) →
/// `None`. Like the host, no default ships in the binary; both must be
/// configured for self-update to do anything.
fn update_repo() -> Option<String> {
    crate::infra_config::update_repo()
}
...
fn release_download_prefix(gitea_host: &str, repo: &str) -> String {
...
fn parse_latest(body: &str, gitea_host: &str, repo: &str) -> Option<LatestAsset> {
...
pub async fn check_release_update() -> Result<ReleaseUpdate, String> {
    ...
    // No release host/repo configured → quietly idle: supported but never
...
pub async fn apply_release_update(download_url: String, asset_key: String) -> Result<String, String> {
    ...
    if host.is_none() || repo.is_none() {
        return Err(
            "self-update is not configured — set update_gitea_host + update_repo in ~/.cortex/infra.json (or CORTEX_UPDATE_GITEA_HOST / CORTEX_UPDATE_REPO)"
                .to_string(),
        );
    }
    if !is_trusted_download_url(&download_url, host.as_deref(), repo.as_deref()) {
    ...
    if !is_trusted_download_url(&sig_url, host.as_deref(), repo.as_deref()) {
...
mod tests {
    ...
    const REPO: &str = "exampleowner/cortex";
    ...
    fn picks_appimage_asset_and_builds_key_url_and_sig() {
        let l = parse_latest(SAMPLE, HOST, REPO).expect("parse");
        ...
        assert_eq!(
            l.url,
            "http://git.example.com:3000/exampleowner/cortex/releases/download/v0.0.1-20260531/Cortex_0.0.1_amd64.AppImage"
        );
        assert_eq!(
            l.sig_url.as_deref(),
            Some("http://git.example.com:3000/exampleowner/cortex/releases/download/v0.0.1-20260531/Cortex_0.0.1_amd64.AppImage.sig")
        );
    }
    ...
    fn no_appimage_asset_returns_none() {
        ...
        assert!(parse_latest(body, HOST, REPO).is_none());
    }

    #[test]
    fn empty_or_garbage_returns_none() {
        assert!(parse_latest("[]", HOST, REPO).is_none());
        assert!(parse_latest("not json", HOST, REPO).is_none());
        assert!(parse_latest("{}", HOST, REPO).is_none());
    }
    ...
    fn trusts_only_configured_gitea_release_urls() {
        ...
        assert!(!is_trusted_download_url(
            "http://git.example.com:3000/exampleowner/cortex/releases/download/../../etc/passwd",
            Some(HOST),
            Some(REPO)
        ));
        assert!(!is_trusted_download_url(
            "http://evil@git.example.com:3000/exampleowner/cortex/releases/download/v1/x.AppImage",
            Some(HOST),
            Some(REPO)
        ));
    }
    ...
    fn key_changes_when_asset_reuploaded() {
        ...
        let k1 = parse_latest(SAMPLE, HOST, REPO).unwrap().key;
        let k2 = parse_latest(&reup, HOST, REPO).unwrap().key;
        assert_ne!(k1, k2);
    }
}
