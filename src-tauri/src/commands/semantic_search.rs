...
pub async fn semantic_memory_search(
    ...
    search_notes(&ollama_base, vault, active, &query, limit.unwrap_or(10)).await
}

/// Vault semantic-retrieval core, callable outside the Tauri command (the
/// unified-RAG path reuses it). Same hybrid approach: lexical pre-filter
/// (Obsidian REST or filesystem walk) then embedding cosine re-rank, degrading
/// to lexical order on any Ollama failure.
    ...
    let mut hits: Vec<SemanticHit> = match embed_text(ollama_base, &model, query).await {
        Ok(q_vec) => {
            ...
            let mut out: Vec<SemanticHit> = futures::stream::iter(scored.iter().cloned())
                .map(|(path, snip, _lex)| {
                    let q = Arc::clone(&q);
                    let base = ollama_base.to_string();
                    let m = model.clone();
                    async move {
