...
pub async fn list_models(state: State<'_, AppState>) -> Result<Vec<ModelEntry>, String> {
    ...
    // Model Fabric endpoints — probe each enabled endpoint's `/v1/models`
    // (through the Tailscale proxy) and surface its models, namespaced
    // `fabric-<slug>:<model>` so `route()` sends them to that endpoint's adapter
    // (which runs e.g. real Claude/Codex CLI sessions on a homelab server).
    for ep in crate::agents::custom_endpoint::load_endpoints() {
        if !ep.enabled {
            continue;
        }
        let probe = crate::agents::custom_endpoint::probe(&ep.base_url).await;
        if !probe.ok {
            continue;
        }
        for m in probe.models.into_iter().take(40) {
            out.push(ModelEntry {
                id: format!("{}:{}", ep.id, m),
                label: format!("{} · {}", ep.label, m),
                source: ep.id.clone(),
                available: true,
            });
        }
