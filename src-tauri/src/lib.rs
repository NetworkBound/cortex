...
pub mod skills;
pub mod sys;
// Loopback→WSL-IP SOCKS bridge. Used by the "Cortex Home" thin client AND the
// full app when it's pointed at a loopback external SOCKS (WSL NAT mode doesn't
// forward Windows 127.0.0.1 to the WSL proxy on its own).
...
pub fn run() {
    ...
    if let Some(addr) = crate::tailscale::external_socks_addr() {
        ...
        // Auto-(re)start Tailscale-in-WSL so the proxy is actually LIVE on launch.
        // WSL Tailscale does not persist across a WSL/VM teardown and its IP can
        // change between sessions, so a configured proxy addr is often stale/dead
        // at startup — which is why the gateway/usage/ollama silently fail. This
        // reconnects it (idempotent: reuses the install + node state, just brings
        // the daemon back + refreshes the SOCKS addr). Runs off the startup thread
        // so a slow bring-up never blocks the window. First-time auth still needs
        ...
        if crate::tailscale::wsl::available() {
            std::thread::spawn(|| match crate::tailscale::wsl::setup() {
                Ok(st) => match st.login_url {
                    ...
                    None => tracing::info!(
                        "tailscale (WSL): auto-started — proxy {:?}, tailnet {:?}",
                        st.proxy_addr,
                        st.tailnet_ip
                    ),
                },
        ...
        // forward there on its own. When the configured proxy is loopback, run an
        // in-process bridge: bind 127.0.0.1:1055 and forward to the live
        // <wsl-ip>:1055 (re-resolved per connection). Without this the full app
        // silently can't reach home (gateway/ollama/fabric all fail). The
        // remote_client build runs its own bridge below, so skip it here.
        #[cfg(not(feature = "remote_client"))]
        if crate::tailscale::wsl::available()
            && (addr.starts_with("127.0.0.1:") || addr.starts_with("localhost:"))
        {
            let listen = addr.clone();
            std::thread::spawn(move || crate::remote_forward::run(&listen));
        }
        ...
        if let Err(e) = crate::tailscale::manager::start(key, &socks, "cortex") {
            // Missing sidecar binary etc. — surface as Error status, never panic.
            tracing::warn!("embedded tailscale autostart skipped: {e}");
            *crate::tailscale::shared().status.write() =
    ...
    // "Cortex Home" (remote_client) build: route EVERY webview through a fixed
    // local SOCKS5 address and bridge it, in-process, to the dynamic WSL
    // Tailscale proxy. WebView2's proxy is set once at environment-init (before
    // any window) and can't be per-window or updated live — so a stable local
    // hop is the only way to reach the tailnet-only home server from this
    // locked-down host. Must run before the builder creates the first webview.
    #[cfg(feature = "remote_client")]
    {
        std::env::set_var(
        ...
        std::thread::spawn(|| {
            // Bring the tunnel up (installs + --accept-routes; idempotent), then
            // bridge 127.0.0.1:1055 -> the live WSL proxy. run() resolves the WSL
            // IP itself and waits for the proxy to accept connections, so it's
            // robust to a slow first-run bring-up and a changing WSL IP.
            let _ = crate::tailscale::wsl::setup();
            crate::remote_forward::run("127.0.0.1:1055");
        });
    }
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::agents::list_openai_compat_providers,
            ...
            commands::settings::get_outcome_routing,
            commands::settings::set_outcome_routing,
            commands::settings::get_session_budget,
            commands::settings::set_session_budget,
            ...
            commands::observability::run_replay,
            commands::observability::export_run_replay,
            ...
            commands::diagnostics::export_audit_log,
            commands::safe_mode::safe_mode_status,
            commands::safe_mode::set_safe_mode,
            commands::safe_mode::get_command_policy,
            commands::safe_mode::set_command_policy,
            commands::safe_mode::test_command_policy,
            commands::safe_mode::apply_ci_safe_profile,
            commands::safe_mode::ci_safe_policy_preview,
            commands::safe_mode::get_builtin_command_rules,
            ...
            commands::vault_analysis::analyze_vault,
            commands::vault_auto_sort::vault_auto_sort,
            commands::prompt_enhance::enhance_prompt,
            commands::prompt_enhance::generate_agent_instructions,
            ...
            commands::sessions::export_session_to_vault,
            commands::chat_semantic::semantic_chat_reindex,
            commands::chat_semantic::semantic_chat_search,
            commands::brain_rag::brain_rag,
            commands::brain_rag::brain_stale_notes,
            commands::brain_rag::brain_memory_duplicates,
            ...
            commands::chat_import::import_chat_file,
            commands::chat_import::import_chat_pull,
            commands::history_sync::history_sync_set_enabled,
            commands::history_sync::history_sync_status,
            commands::history_sync::history_sync_now,
            commands::history_sync::history_sync_connect,
            ...
            commands::eval_harness::list_retrieval_eval_tasks,
            commands::eval_harness::list_retrieval_eval_reports,
            commands::eval_harness::run_retrieval_eval,
            ...
            commands::issues::issues_import,
            commands::issues::issue_run_in_lane,
            commands::issues::issue_pr_preview,
            commands::issues::issue_open_pr,
            ...
            commands::workflows::export_workflow,
            ...
            commands::mcp::mcp_probe_runtimes,
            ...
            commands::tailscale::ts_enable,
            commands::tailscale::ts_disable,
        ...
        .setup(move |app| {
            ...
            if let Some(win) = tauri::Manager::get_webview_window(app, "main") {
                ...
                if webview_data_should_clear() {
                    // Clears the main (shared) WebContext only. History-sync
                    // provider sign-ins live in their own persistent WebView2
                    // profiles (see history_sync::webview_fetch — each gets a
                    // dedicated `data_directory`), so they are intentionally NOT
                    // wiped here and survive app updates.
                    tracing::info!("app version changed — clearing stale main webview data");
            ...
            // Automatic chat-history sync: one background loop per provider the
            // user has enabled in Settings → History Sync. No-op until enabled;
            // each loop syncs immediately then re-syncs on a schedule, reusing
            // the chat_import pull+import pipeline (incremental via dedup).
            {
                use tauri::Manager;
                let store = app.state::<TracingStore>().inner().clone();
                crate::history_sync::scheduler::spawn_for_enabled(app.handle().clone(), store);
            }
            ...
            // Continuous auto-index ("always attached"): keep the unified
            // semantic index — chat messages AND vault/memory notes — current so
            // "Ask your brain" always covers the whole brain without a manual
            // reindex. Runs an initial pass shortly after launch, then every
            // ~30 min so notes added or chats synced mid-session get embedded
            // without a restart. Incremental (only new/changed items embed),
            // best-effort (a missing Ollama just logs and retries next tick), and
                ...
                tauri::async_runtime::spawn(async move {
                    ...
                    loop {
                        ...
                        if base.trim().is_empty() {
                            tracing::debug!(
                                target: "cortex::rag",
                                "auto-index idle: no Ollama base URL yet"
                            );
                        } else {
                            match crate::commands::chat_semantic::reindex_all(
                                &store, &base, vault, None,
                            )
                            .await
                            {
                                Ok(r) => tracing::info!(
                                    ...
                                    "auto-index pass complete"
                                ),
                                Err(e) => tracing::info!(
                                    target: "cortex::rag",
                                    error = %e,
                                    "auto-index pass skipped"
                                ),
                            }
                        }
...
/// embedded mobile server can run on a VM. Mirrors the registry population in
/// [`run`] (gateway adapter + every catalog CLI/provider/runtime/Ollama
/// adapter), minus the Tauri window/plugin/setup machinery.
///
...
/// path. If the registration list in [`run`] changes, update it here too. The
/// `standalone`/cloud-mode branch is intentionally omitted here (headless serve
/// targets the default homelab gateway build); add it if a cloud headless build
/// is ever needed.
pub fn build_headless_state() -> (AppState, TracingStore) {
    ...
    // Issue 008: same audit sink as the full app (first `set` wins, so this
    // is a no-op when the desktop run() already installed one).
    crate::mcp::chat_tools::set_audit_store(store.clone());
