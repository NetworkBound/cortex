...
pub fn discover_projects(vault_root: Option<PathBuf>) -> Vec<ProjectMeta> {
    ...
    // When the same project name exists as both a WSL UNC path and a native
    // Windows path, keep only the native one — CLI agents (Codex, Claude, …)
    ...
    #[cfg(windows)]
    {
        ...
        let native_names: std::collections::HashSet<String> = out
            .iter()
            .filter(|p| !is_wsl(p) && p.kind == "code")
            .map(|p| p.name.clone())
            .collect();
        out.retain(|p| !is_wsl(p) || !native_names.contains(&p.name));
    }
...
fn discover_code_projects() -> Vec<ProjectMeta> {
    ...
    // this, the sidebar shows empty on Windows for anyone whose repos live
    // under `/home/<user>/projects` on WSL.
    ...
    #[cfg(windows)]
    {
        ...
        // distro's actual home dirs instead of guessing a literal name.
        let user = std::env::var("USERNAME").ok().map(|u| u.to_lowercase());
        for distro in ["Ubuntu", "Ubuntu-24.04", "Ubuntu-22.04", "Debian"] {
            if let Some(u) = &user {
                let p = PathBuf::from(format!("\\\\wsl.localhost\\{distro}\\home\\{u}\\projects"));
                if p.exists() {
            ...
            if let Ok(rd) = std::fs::read_dir(&home_root) {
                for p in rd.flatten().map(|e| e.path().join("projects")) {
    ...
    // Shallow scan of the home directory (depth 1 only) to discover repos
    // directly under ~ (e.g. C:\Users\<user>\cortex) that aren't under the
    // deeper ~/projects/* tree. This ensures Windows-native repos are found
    // alongside WSL-discovered ones, and CLI agents get a native-path cwd
    // instead of a slow \\wsl.localhost UNC path.
