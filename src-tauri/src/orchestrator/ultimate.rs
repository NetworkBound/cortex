...
async fn run_adapter_in_worktree(
    ...
    // Ultimate's isolated-worktree fan-out has no approval-resume channel
    // (unlike the interactive chat path, which resolves ApprovalRequest via
    // `chat.rs`'s per-run_id wait). If a sub-agent needs approval mid-run,
    // there is nothing here to grant it — the most likely outcome is the
    // adapter blocks until `CODE_WORKER_TIMEOUT` fires. This flag lives
    // OUTSIDE `driven` (on the heap, shared via Arc) so the timeout branch
    // below can still read it even when `driven` itself gets cancelled by
    // `tokio::time::timeout` — otherwise the most common failure shape (timed
    // out while silently stuck on a dropped approval request) would report
    // only an unexplained "timed out after Ns", exactly the kind of
    // indistinguishable-from-a-hang UX this is meant to prevent.
    let saw_approval_request = std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false));
    let saw_approval_request_w = saw_approval_request.clone();
        ...
        while let Some(ev) = rx.recv().await {
            match ev {
                ...
                AgentEvent::ApprovalRequest { .. } => {
                    saw_approval_request_w.store(true, std::sync::atomic::Ordering::Relaxed);
                }
                _ => {}
            }
        }
    ...
    let result = tokio::time::timeout(CODE_WORKER_TIMEOUT, driven).await;
    let approval_note = if saw_approval_request.load(std::sync::atomic::Ordering::Relaxed) {
        " (it also requested approval for a tool call mid-run — Ultimate's isolated worktree fan-out has no way to grant that; this candidate cannot proceed past an approval-gated action)"
    } else {
        ""
    };
    match result {
        Ok(inner) => inner.map_err(|e| format!("{e}{approval_note}")),
        Err(_) => Err(format!(
            "timed out after {}s{approval_note}",
            CODE_WORKER_TIMEOUT.as_secs()
        )),
    }
}
