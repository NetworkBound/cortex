...
pub struct Team {
    ...
    /// Structured verdict parsed from the synthesis pass's verification block
    /// (orchestration slice 5+). `None` until a synthesized run produces a
    /// parseable verdict; surfaced as a pass/fail scorecard on the dashboard so
    /// "did it work?" is answered at a glance rather than buried in prose.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub verdict: Option<Verdict>,
}

/// A synthesis verification verdict: the manager's structured judgment of
/// whether the merged result meets the goal. Parsed tolerantly from a fenced
/// JSON block the synthesis prompt requests (LobeChat "delivery checker"
/// pattern) — a missing/garbled block just leaves [`Team::verdict`] `None`.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct Verdict {
    /// Overall pass/fail — did the team actually accomplish the goal?
    pub pass: bool,
    /// Confidence/quality score in `0..=100` (clamped on parse).
    pub score: u8,
    /// Per-check line items backing the verdict.
    #[serde(default)]
    pub checks: Vec<VerdictCheck>,
}

/// One line item in a [`Verdict`] — a named check and whether it held.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
...
/// Attach the synthesis verification verdict (slice 5+). Called once after the
/// synthesis pass parses a verdict from its output; `None` leaves it unset.
pub fn set_verdict(team_id: &str, verdict: Verdict) -> anyhow::Result<Team> {
    let mut team =
        get_team(team_id).ok_or_else(|| anyhow::anyhow!("team '{team_id}' not found"))?;
    team.verdict = Some(verdict);
    save_team(&team)?;
    Ok(team)
}
