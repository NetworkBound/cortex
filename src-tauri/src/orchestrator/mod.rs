...
///
/// `d.available` alone isn't the whole story for adapters that can only
/// judge real reachability via a network probe (local-runtime endpoints —
/// see `agents::local_runtime`'s doc comment): their synchronous descriptor
/// always reports `available: true` and the real check lives in async
/// `health_check()`. This ANDs in `registry.known_reachable(id)` — the
/// most recent observed result from that check, recorded whenever the UI's
/// periodic health poll actually probes an adapter (see
/// `commands::agents::check_agent_health` / `Registry::health_cache`'s doc
/// comment). No poll has ever run for `id` → `None` → `unwrap_or(true)` →
/// behavior identical to before this existed. This only ever narrows
/// `available: true` to `false` when we have real, if possibly slightly
/// stale, evidence the adapter is down — never the reverse, so it can't make
/// an actually-reachable adapter look unavailable.
fn adapter_available(registry: &Registry, id: &str) -> bool {
    let descriptor_available = registry
        .list_descriptors()
        .into_iter()
...
/// [`route`] plus an optional cost-per-success hint (issue 006) consulted by
/// the DEFAULT branch **only**. `None` — the default-off state, and what
/// [`route`] always passes — makes this byte-identical to `route` before the
/// hint existed. Even with a hint, every earlier branch (explicit pick, exact
/// adapter id, fabric prefix, CLI slugs, `ollama:`) returns before the hint is
/// looked at, and a request that names ANY model skips it too — outcome
/// routing must never move a model-carrying request off the adapter that
/// serves that model, and must never override a user pick.
pub fn route_with_outcome(
    req: &ChatRequest,
    registry: &Registry,
    preferred_agent: Option<String>,
    outcome: Option<&cost_router::OutcomePick>,
) -> RoutingDecision {
    ...
    // Model Fabric endpoint models are namespaced `fabric-<slug>:<model>` (from
    // `list_models`). Route to that endpoint's adapter, which strips the prefix
    // to pick the concrete model on the homelab server (real Claude/Codex CLI).
    if let Some(model) = req.model.as_deref() {
        if let Some((ep, _rest)) = model.trim().split_once(':') {
            if ep.starts_with(crate::agents::custom_endpoint::FABRIC_PREFIX)
                && adapter_available(registry, ep)
            {
                return RoutingDecision {
                    agents: vec![ep.to_string()],
                    stripped_message: msg,
                    reason: format!("model-route → fabric {ep}"),
                };
            }
        }
    }
    ...
    // DEFAULT branch. The opt-in cost-per-success hint (issue 006) applies
    // here and ONLY here, and only for a bare request (no model requested —
    // a model that fell through the branches above still names what to run,
    // and the default target is the right server for it). The hinted adapter
    // is re-checked against live availability; a dead hint falls through to
    // today's exact default behavior.
    if let Some(pick) = outcome {
        let no_model = req.model.as_deref().map_or(true, |m| m.trim().is_empty());
        if no_model && adapter_available(registry, &pick.agent_id) {
            return RoutingDecision {
                agents: vec![pick.agent_id.clone()],
                stripped_message: msg,
    ...
    let target = if adapter_available(registry, &default) {
        default.clone()
    } else {
        registry
            .list_descriptors()
            .into_iter()
            .find(|d| adapter_available(registry, &d.id))
            .map(|d| d.id)
...
mod route_tests {
    ...
    #[test]
    fn adapter_available_respects_health_cache_override() {
    ...
    #[test]
    fn adapter_available_never_widens_a_descriptor_reporting_unavailable() {
        // The cache is an override that can only narrow true→false, never
        // the reverse — a descriptor reporting unavailable must stay
        // unavailable even if a stale health poll once said "healthy".
        let reg = reg_with(&[("dead-adapter", false)]);
        reg.record_health("dead-adapter", true);
        assert!(!adapter_available(&reg, "dead-adapter"));
    }

    #[test]
    fn default_route_falls_through_when_gateway_is_known_unreachable() {
        // Both descriptors report available:true (unchanged, cheap signal),
        // but gateway-remote's last real health poll observed it down.
        // The default-fallback picker must skip it and land on the other
        // available (and not known-unreachable) adapter instead.
        let reg = reg_with(&[("gateway-remote", true), ("ollama", true)]);
        reg.record_health("gateway-remote", false);
        let d = route(&req_for(None), &reg, None);
    ...
    fn hint(id: &str) -> cost_router::OutcomePick {
    ...
    fn no_hint_is_byte_identical_to_route() {
        ...
        for (model, preferred) in [
            ...
            (Some("opus"), None),
            (Some("gpt-5.5"), None),
            (Some("ollama:llama3.2"), None),
            (Some("does-not-exist"), None),
        ] {
            let req = req_for(model);
            assert_eq!(
                route_with_outcome(&req, &reg, preferred.clone(), None),
                route(&req, &reg, preferred),
                "model={model:?}"
            );
        }
    }
    ...
    #[test]
    fn outcome_hint_steers_only_the_bare_default_branch() {
        let reg = reg_with(&[("claude-cli", true), ("gateway-remote", true)]);
        let h = hint("claude-cli");
        let d = route_with_outcome(&req_for(None), &reg, None, Some(&h));
        assert_eq!(d.agents, vec!["claude-cli".to_string()]);
        // The outcome rationale rides the existing routing_reason.
        assert_eq!(d.reason, h.reason);
    }

    #[test]
    fn outcome_hint_never_overrides_an_explicit_pick() {
        let reg = reg_with(&[("claude-cli", true), ("gateway-remote", true)]);
        let h = hint("claude-cli");
        let req = req_for(None);
        let with = route_with_outcome(&req, &reg, Some("gateway-remote".to_string()), Some(&h));
        let without = route(&req, &reg, Some("gateway-remote".to_string()));
        assert_eq!(with, without, "explicit pick must be byte-identical");
        assert_eq!(with.agents, vec!["gateway-remote".to_string()]);
        assert_eq!(with.reason, "explicit pick: gateway-remote");
    }

    #[test]
    fn outcome_hint_never_touches_model_or_cli_branches() {
        // Every model-carrying request — CLI slugs, exact adapter ids, ollama
        // prefixes, and even a gateway-catalog slug that lands in the default
        // branch — must be byte-identical with and without a hint.
        let reg = reg_with(&[
            ("claude-cli", true),
            ("gateway-remote", true),
            ("ollama", true),
        ...
        for model in ["opus", "claude-opus-4-8", "gpt-5.5", "ollama:llama3.2", "e2e-fake"] {
            ...
            assert_eq!(with, without, "{model} must ignore the hint");
        }
        // In particular: "gpt-5.5" (no codex-cli here) falls to the default
        // branch but STILL names a model — it must stay on the gateway that
        // serves it, never the hinted adapter.
        let d = route_with_outcome(&req_for(Some("gpt-5.5")), &reg, None, Some(&h));
        assert_eq!(d.agents, vec!["gateway-remote".to_string()]);
    }

    #[test]
    fn outcome_hint_for_unavailable_adapter_falls_back_to_default() {
        // Hinted adapter is registered but down → today's exact default.
        let reg = reg_with(&[("claude-cli", false), ("gateway-remote", true)]);
        let h = hint("claude-cli");
        let req = req_for(None);
        let with = route_with_outcome(&req, &reg, None, Some(&h));
        assert_eq!(with, route(&req, &reg, None));
        assert_eq!(with.agents, vec!["gateway-remote".to_string()]);
        // Same when a health poll observed the hinted adapter unreachable.
        let reg2 = reg_with(&[("claude-cli", true), ("gateway-remote", true)]);
        reg2.record_health("claude-cli", false);
        let with2 = route_with_outcome(&req, &reg2, None, Some(&hint("claude-cli")));
        assert_eq!(with2.agents, vec!["gateway-remote".to_string()]);
    }
}
