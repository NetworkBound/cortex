...
const SENSITIVE_PATH_FRAGMENTS: &[&str] = &[
    ...
    // MCP server registry — per-server `env` maps can carry plaintext API
    // tokens (Agent E MED finding), so an agent must never read it back
    // through a tool call. Substring matching also covers the atomic-save
    // temp file (`mcp-servers.json.tmp`).
    "/.cortex/mcp-servers.json",
    "\\.cortex\\mcp-servers.json",
    // Encrypted provider key vault. Ciphertext today, but there is zero
    // legitimate reason for a tool call to read it.
    "/.cortex/keys.enc",
    "\\.cortex\\keys.enc",
];
...
/// Issue 008 — tier gate for MCP-origin tools, keyed on the server-local
/// (bare) tool name. Additive: [`tier_allows`] is untouched and keeps gating
/// every built-in tool exactly as before.
///
/// MCP tool names that classify under the existing read/write token model
/// (`read_file`, `write_file`, `fetch`, …) delegate to [`tier_allows`]
/// verbatim, inheriting the sensitive-path floor and root confinement. The
/// difference is the *unclassified* remainder (`get_forecast`,
/// `create_issue`-less names like `echo`): `tier_allows` fails closed on
/// those, which is right for filesystem-shaped built-ins but would make every
/// non-filesystem MCP tool permanently uncallable. Instead they are treated
/// like exec-shaped tools (arbitrary capability inside the server process):
///
///   * `ReadOnly`         — denied (we can't prove the tool doesn't mutate).
///   * `WorkspaceWrite`   — allowed, with the same sensitive-path floor and
///                          every path-shaped argument confined to the root.
///   * `DangerFullAccess` — allowed.
///
/// The per-server MCP trust level (issue 009: trusted/ask/untrusted) still
/// gates every call *after* this tier check — this is one link of the chain,
/// not the whole chain.
pub fn tier_allows_mcp(
    tier: SandboxTier,
    bare_tool: &str,
    payload_json: &str,
    project_root: Option<&Path>,
    ...
    match tier {
        SandboxTier::DangerFullAccess => Ok(()),
        SandboxTier::ReadOnly => Err(format!(
            "tier 'read-only' forbids MCP tool '{bare_tool}' (unclassified tools need workspace-write or higher)"
        )),
        SandboxTier::WorkspaceWrite => {
            if let Some(bad) = touches_sensitive_path(payload_json) {
                return Err(format!(
            ...
            if let Some(root) = project_root {
                if let Some(bad) = collect_paths(payload_json)
                    .iter()
                    .find(|p| !path_inside(root, p))
                {
                    return Err(format!(
                        "tier 'workspace-write' refused: MCP arg path '{bad}' escapes project root"
                    ));
                }
            }
            Ok(())
...
mod tests {
    ...
    fn read_only_denies_reads_of_sensitive_credential_paths() {
        ...
        let sensitive = [
            ...
            "/home/user/.cortex/mcp-servers.json",
            "/home/user/.cortex/keys.enc",
        ];
    ...
    /// Issue 009 security fix: the MCP server registry carries plaintext MCP
    /// env tokens, so an agent must not be able to read it via any tool call
    /// under either restricted tier — as a JSON path arg, via a Windows-style
    /// path, or as a bare word inside a "provably read-only" shell command.
    #[test]
    fn agent_cannot_read_mcp_token_registry() {
        let p: Option<&Path> = None;
        for t in [SandboxTier::ReadOnly, SandboxTier::WorkspaceWrite] {
            for path in [
                "~/.cortex/mcp-servers.json",
                "/home/user/.cortex/mcp-servers.json",
                "C:\\Users\\user\\.cortex\\mcp-servers.json",
                // The atomic-save temp file must be covered too.
                "/home/user/.cortex/mcp-servers.json.tmp",
            ] {
                let payload = serde_json::json!({ "path": path }).to_string();
                assert!(
                    tier_allows(t, "read_file", &payload, p).is_err(),
                    "expected deny for MCP registry path: {path} under {t:?}"
                );
            }
        }
    ...
    #[test]
    fn mcp_tier_gate_delegates_for_classified_names() {
        let p: Option<&Path> = None;
        // Read-shaped → allowed under ReadOnly, like any built-in read tool.
        assert!(tier_allows_mcp(SandboxTier::ReadOnly, "read_file", "{}", p).is_ok());
        assert!(tier_allows_mcp(SandboxTier::ReadOnly, "fetch", "{}", p).is_ok());
        // Write-shaped → denied under ReadOnly.
        assert!(tier_allows_mcp(SandboxTier::ReadOnly, "write_file", "{}", p).is_err());
        // Sensitive-path floor still applies to reads.
        let payload = serde_json::json!({ "path": "~/.ssh/id_rsa" }).to_string();
        assert!(tier_allows_mcp(SandboxTier::ReadOnly, "read_file", &payload, p).is_err());
    }

    /// Issue 008: unclassified MCP names (non-filesystem-shaped tools) are
    /// exec-like — denied under ReadOnly, allowed under WorkspaceWrite with
    /// path confinement + the sensitive-path floor, and unrestricted under
    /// DangerFullAccess.
    #[test]
    fn mcp_tier_gate_unclassified_names_are_exec_like() {
        let p: Option<&Path> = None;
        assert!(tier_allows_mcp(SandboxTier::ReadOnly, "get_forecast", "{}", p).is_err());
        assert!(tier_allows_mcp(SandboxTier::WorkspaceWrite, "get_forecast", "{}", p).is_ok());
        assert!(tier_allows_mcp(SandboxTier::DangerFullAccess, "get_forecast", "{}", p).is_ok());

        // WorkspaceWrite: sensitive-path floor holds even for unclassified
        // names — an MCP tool must not be handed a credential-store path.
        let payload = serde_json::json!({ "target": "~/.aws/credentials" }).to_string();
        assert!(tier_allows_mcp(SandboxTier::WorkspaceWrite, "get_forecast", &payload, p).is_err());

        // WorkspaceWrite + project root: path-shaped args are confined.
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path();
        let inside = serde_json::json!({ "path": root.join("x.txt") }).to_string();
        assert!(
            tier_allows_mcp(SandboxTier::WorkspaceWrite, "do_thing", &inside, Some(root)).is_ok()
        );
        let outside = r#"{"path":"/etc/passwd"}"#;
        assert!(
            tier_allows_mcp(SandboxTier::WorkspaceWrite, "do_thing", outside, Some(root)).is_err()
        );
    }
