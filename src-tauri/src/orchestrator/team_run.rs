...
/// Extract the LAST balanced `{...}` object span from `raw` (brace-aware,
/// string/escape-aware). "Last" because the synthesis prompt asks for the
/// verdict JSON at the very end, after prose that may itself contain braces.
fn extract_last_json_object(raw: &str) -> Option<String> {
    let bytes = raw.as_bytes();
    // Walk back to the final top-level '}' then match its opening '{'.
    let end = raw.rfind('}')?;
    let mut depth = 0usize;
    let mut in_str = false;
    let mut escape = false;
    // Scan forward from each '{' candidate is O(n^2); instead scan the whole
    // string once, tracking the start of the object that closes at `end`.
    ...
    for (i, c) in raw.char_indices() {
        ...
        match c {
            ...
            '}' if !in_str => {
                ...
                if depth == 0 && i == end {
                    return start.map(|s| raw[s..=i].to_string());
                }
            }
            _ => {}
        }
...
/// Parse the synthesis verdict from the merged output's trailing JSON block.
/// Tolerant of fences/prose around it and of a missing `checks` array; returns
/// `None` when no parseable verdict is present (verdict is best-effort UI
/// sugar, never load-bearing). `score` is clamped to 0..=100.
fn parse_verdict(merged: &str) -> Option<teams::Verdict> {
    ...
    if should_synthesize(&outcomes) {
        ...
        .await
        {
            Ok(merged) if !merged.trim().is_empty() => {
                ...
                // Parse the trailing verdict JSON into a status card (best-effort:
                // a missing/garbled block just leaves the team's verdict unset).
                if let Some(v) = parse_verdict(&merged) {
                    let _ = teams::set_verdict(team_id_ref, v);
                }
...
fn build_synthesis_prompt(team: &Team, goal: &str, outcomes: &[WorkerSummary]) -> String {
    ...
    format!(
        "You are \u{201c}{manager}\u{201d}, the manager of team \u{201c}{name}\u{201d}. Your \
         ...
         Finally, end your reply with a fenced ```json block (and nothing after it) holding a \
         verdict for a status card: {{\"pass\": <true|false>, \"score\": <0-100>, \"checks\": \
         [{{\"name\": \"<short check>\", \"pass\": <true|false>, \"note\": \"<why, optional>\"}}]}}. \
         `pass` is whether the goal is genuinely met; `score` is your confidence in the merged \
         result; include one check per worker or per goal requirement.",
        manager = team.manager_role,
        name = team.name,
    )
}
...
mod tests {
    ...
    // ---- synthesis verdict parsing ----

    #[test]
    fn synthesis_prompt_requests_a_verdict_block() {
        let t = team_with(&[("wkr-a", "coder"), ("wkr-b", "tester")]);
        let outcomes = vec![
            summary("coder", "done", "did x"),
            summary("tester", "done", "tested x"),
        ];
        let p = build_synthesis_prompt(&t, "ship it", &outcomes);
        assert!(p.contains("```json"));
        assert!(p.contains("\"pass\"") && p.contains("\"score\"") && p.contains("\"checks\""));
    }

    #[test]
    fn parses_verdict_from_trailing_fenced_json() {
        let merged = "Merged result here.\n\n## Verification\nLooks good.\n\n```json\n\
            {\"pass\": true, \"score\": 92, \"checks\": [\
            {\"name\": \"compiles\", \"pass\": true}, \
            {\"name\": \"tests\", \"pass\": false, \"note\": \"1 flaky\"}]}\n```";
        let v = parse_verdict(merged).expect("verdict");
        assert!(v.pass);
        assert_eq!(v.score, 92);
        assert_eq!(v.checks.len(), 2);
        assert_eq!(v.checks[1].name, "tests");
        assert!(!v.checks[1].pass);
        assert_eq!(v.checks[1].note.as_deref(), Some("1 flaky"));
    }

    #[test]
    fn verdict_score_is_clamped_and_checks_optional() {
        // Over-100 score clamps; missing checks defaults to empty.
        let v = parse_verdict("prose {\"pass\": false, \"score\": 250}").expect("verdict");
        assert!(!v.pass);
    ...
    #[test]
    fn verdict_ignores_prose_braces_and_takes_the_last_object() {
        // A brace-y prose body {like this} before the real trailing verdict.
        let merged = "Result mentions a struct Foo { bar } inline.\n```json\n{\"pass\": true, \"score\": 40}\n```";
        let v = parse_verdict(merged).expect("verdict");
        assert!(v.pass);
    ...
    #[test]
    fn no_verdict_block_yields_none() {
        assert!(parse_verdict("Just prose, no JSON at all.").is_none());
        assert!(parse_verdict("").is_none());
        // Malformed JSON (missing required `pass`) → None, never a panic.
