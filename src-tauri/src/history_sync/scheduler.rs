...
/// Providers whose background loop is already running. Prevents repeated
/// enables / Connects (each of which calls [`spawn_provider_loop`]) from leaking
/// a second concurrent loop for the same provider.
fn loops_running() -> &'static Mutex<HashSet<String>> {
    static S: OnceLock<Mutex<HashSet<String>>> = OnceLock::new();
    S.get_or_init(|| Mutex::new(HashSet::new()))
}

/// Spawn background sync loops for every provider that is currently enabled in
/// the persisted config. Call once from the Tauri setup hook. The [`AppHandle`]
/// lets a loop reuse a persisted webview sign-in via a hidden headless re-sync.
///
/// [`AppHandle`]: tauri::AppHandle
pub fn spawn_for_enabled(app: tauri::AppHandle, store: TracingStore) {
    let cfg = super::config::load();
    for provider in cfg.enabled_providers() {
        spawn_provider_loop(app.clone(), provider, store.clone());
    }
}
...
pub fn spawn_provider_loop(app: tauri::AppHandle, provider: String, store: TracingStore) {
    // One loop per provider: bail if one is already running so repeated
    // enable/Connect calls can't leak concurrent loops.
    {
        let Ok(mut running) = loops_running().lock() else {
            return;
        };
        if !running.insert(provider.clone()) {
            return; // a loop for this provider already exists
        }
    }
    tauri::async_runtime::spawn(async move {
        // Immediate first sync on enable.
        run_once(&app, &provider, &store).await;
...
/// One sync pass with structured (secret-free) logging. Uses the full fallback
/// chain (browser/keychain → hidden headless webview reuse).
async fn run_once(app: &tauri::AppHandle, provider: &str, store: &TracingStore) {
    match super::sync_provider_auto(provider, app, store).await {
