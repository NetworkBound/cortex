...
mod non_windows_impl {
    ...
    /// Firefox stores cookie values in plaintext in `cookies.sqlite` under each
    /// profile in `~/.mozilla/firefox/*/`. Best-effort: first matching profile.
    fn firefox_cookie(provider: WebProvider) -> Result<String, String> {
        let base = firefox_profiles_dir().ok_or_else(|| "no Firefox profiles dir".to_string())?;
        let entries = std::fs::read_dir(&base).map_err(|e| format!("read profiles: {e}"))?;
        let mut last = String::from("no profile had the cookie");
        for entry in entries.flatten() {
            let db = entry.path().join("cookies.sqlite");
            if !db.exists() {
                continue;
            }
            match read_firefox(&db, provider.domains(), provider.cookie_name()) {
                Ok(v) => return Ok(v),
                Err(e) => last = e,
            }
        }
    ...
    fn firefox_profiles_dir() -> Option<PathBuf> {
        ...
        // Linux path; macOS uses "Library/Application Support/Firefox/Profiles".
    ...
    fn read_firefox(db: &Path, domains: &[&str], name: &str) -> Result<String, String> {
        ...
        let _guard = super::TempFileGuard(tmp.clone());
        let conn = rusqlite::Connection::open_with_flags(
            &tmp,
            rusqlite::OpenFlags::SQLITE_OPEN_READ_ONLY | rusqlite::OpenFlags::SQLITE_OPEN_NO_MUTEX,
        )
