...
/// Sync with the full fallback chain: browser/keychain auto-detect first; if that
/// yields nothing BUT the user previously signed in via the interactive webview
/// (a persistent profile exists), reuse that stored session in a HIDDEN headless
/// webview — automatic re-sync with no credential ever crossing into Rust. Used
/// by the background scheduler and the manual "Sync now" command (both have an
/// [`AppHandle`]). Degrades safely: a headless failure (e.g. expired session)
/// becomes [`SyncOutcome::NeedsLogin`], never an app-level error.
///
/// [`AppHandle`]: tauri::AppHandle
pub async fn sync_provider_auto(
    provider: &str,
    app: &tauri::AppHandle,
    store: &TracingStore,
) -> Result<SyncOutcome, String> {
    match sync_provider(provider, store).await? {
        SyncOutcome::Imported { result, source } => Ok(SyncOutcome::Imported { result, source }),
        SyncOutcome::NeedsLogin => {
            let web = parse_provider(provider)?;
            // Only attempt headless reuse if an interactive sign-in has run before
            // (its isolated WebView2 profile dir exists) — otherwise there is no
            // stored session to reuse and we'd pop a pointless hidden window.
            if !webview_fetch::has_webview_profile(web) {
                return Ok(SyncOutcome::NeedsLogin);
            }
            match webview_fetch::headless_fetch_and_import(web, app, store).await {
                ...
                Err(e) => {
                    ...
                    // so the UI re-offers Connect. The message is secret-free.
                    tracing::info!("history_sync: {provider} headless reuse failed: {e}");
                    stamp_last_sync(web, None);
                    Ok(SyncOutcome::NeedsLogin)
                }
            }
        }
    }
