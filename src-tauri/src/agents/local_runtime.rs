...
impl AgentAdapter for LocalRuntimeAgent {
    fn descriptor(&self) -> AgentDescriptor {
        ...
        // async `health_check` carries the real reachability.
        //
        // UPDATE (2026-07-01): `orchestrator::cost_router::candidates()` does
        // NOT actually consider local-runtime adapters at all — they're not in
        // `aliases::CATALOG` and there's no special-cased block for them the
        // way there is for Ollama — so the originally-suspected "cost_router
        // picks a dead local runtime" scenario cannot happen; that automated
        // fan-out path never routes here. The REAL (much narrower) gap was in
        // `orchestrator::mod.rs::adapter_available`, used by explicit-model-
        // slug routing and the default-fallback picker: it only consulted this
        // synchronous `available: true`, never the real `health_check` result.
        // Fixed via `agents::registry::Registry::health_cache` — a passive
        // cache that `commands::agents::check_agent_health` (the Settings/
        // AgentSidebar UI's existing periodic poll) writes into, and that
        // `adapter_available` now ANDs in as an override (narrows true→false
        // only, never the reverse; no poll yet → unchanged behavior). See
        // that cache's doc comment for the full design.
        AgentDescriptor {
