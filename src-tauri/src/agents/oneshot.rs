...
pub async fn collect_completion(
    ...
    while let Some(ev) = rx.recv().await {
        match ev {
            ...
            // One-shot completions have no approval channel: an adapter that
            // asks and then awaits a resolution would previously have hung
            // this helper forever (the event was silently dropped). Fail
            // fast and abort the run (mirrors the ultimate.rs worktree fix).
            AgentEvent::ApprovalRequest { tool, .. } => {
                err = Some(format!(
                    "tool approval required ({}) — one-shot completions are non-interactive and cannot grant it",
                    tool.as_deref().unwrap_or("unknown tool")
                ));
                abort_run = true;
                break;
            }
    ...
    let run_res: anyhow::Result<()> = if abort_run {
        run_task.abort();
        Ok(())
    } else {
        match run_task.await {
            Ok(r) => r,
            Err(e) => Err(anyhow::anyhow!("agent task join failed: {e}")),
        }
    };
...
mod tests {
    ...
    /// Emits an ApprovalRequest then blocks forever awaiting a resolution
    /// that can never arrive — the old `_ => {}` arm dropped the event and
    /// hung `collect_completion` until the caller's own timeout (if any).
    struct ApprovalHangAdapter;

    #[async_trait]
    impl AgentAdapter for ApprovalHangAdapter {
        fn descriptor(&self) -> AgentDescriptor {
            AgentDescriptor {
                id: "hang".into(),
                label: "hang".into(),
        ...
        async fn run(
            ...
            let _ = tx
                ...
                .await;
            std::future::pending::<()>().await; // awaits a resolution forever
            Ok(())
        }
    }

    #[tokio::test]
    async fn approval_request_fails_fast_instead_of_hanging() {
        let a = Arc::new(ApprovalHangAdapter) as Arc<dyn AgentAdapter>;
        let err = tokio::time::timeout(
            Duration::from_secs(5),
            collect_completion(a, None, "hi".into()),
        )
        .await
        .expect("must not hang")
        .unwrap_err();
        assert!(err.contains("approval required"), "got: {err}");
