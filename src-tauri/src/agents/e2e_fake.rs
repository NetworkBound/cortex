...
impl AgentAdapter for E2eFakeAgent {
    ...
    async fn run(&self, req: ChatRequest, tx: mpsc::Sender<AgentEvent>) -> anyhow::Result<()> {
        ...
        } else if req.message.contains("## Verification")
            || req.message.contains("workers have finished their subtasks")
        {
            // Team manager SYNTHESIS prompt → emit a merged result + a fenced
            // verdict block so the slice-5 verdict-card flow is deterministic.
            let merged = "e2e synthesized merge of all worker outputs.\n\n\
                ## Verification\nAll pieces are consistent and the goal is met.\n\n\
        ...
        } else if req.message.contains("[[e2e:approval-secret]]") {
            // Deterministic risky tool call whose command embeds a live-shaped
            // Bearer token — drives the redaction pipeline (approval prompt +
            // trace store must show [REDACTED]) through the REAL chat_send
            // event loop without needing a gateway.
            let cmd = "curl -H \"Authorization: Bearer sk-testtoken1234567890\" https://api.example.com/v1/models";
            let _ = tx
            ...
            let _ = tx
                .send(AgentEvent::ApprovalRequest {
                    run_id: "e2e-fake-run".to_string(),
                    tool: Some("bash".to_string()),
                    preview: Some(cmd.to_string()),
                    choices: vec!["approve".to_string(), "deny".to_string()],
                    request: serde_json::json!({ "command": cmd }),
                })
                .await;
        } else if req.message.contains("[[e2e:approval-safe]]") {
            // Deterministic approval request for a provably-safe command —
            // drives the trust-matrix auto-approve path (`safe_commands`):
            // trusted project + toggle on → auto-approved (no prompt);
            // untrusted project → must always prompt, whatever the toggles.
            let _ = tx
                .send(AgentEvent::ApprovalRequest {
                    run_id: "e2e-fake-run".to_string(),
                    tool: Some("bash".to_string()),
