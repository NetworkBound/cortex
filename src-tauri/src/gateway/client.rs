...
pub struct ApprovalResponseBody {
    ...
    /// Optional human-readable reason for the decision (e.g. why a run was
    /// denied), surfaced from the mobile approval UI. Omitted from the wire
    /// entirely when `None`, so existing approve/deny calls are unchanged.
    #[serde(skip_serializing_if = "Option::is_none")]
