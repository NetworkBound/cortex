...
pub struct InfraConfig {
    ...
    /// `owner/repo` on that Gitea the self-updater pulls releases from.
    /// Unset → self-update idles, exactly like an unset host (the two are
    /// configured together).
    #[serde(default)]
...
/// `owner/repo` for AppImage self-update:
/// `CORTEX_UPDATE_REPO` → `infra.json` → `None` (self-update idle, matching
/// the unset-host behavior — no owner name is baked into the binary).
pub fn update_repo() -> Option<String> {
    first_configured(
        std::env::var("CORTEX_UPDATE_REPO").ok(),
        load_file().update_repo.as_deref(),
    )
}
