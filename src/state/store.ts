...
interface CortexState {
  ...
  /** Span id `ObservabilityPanel`'s Run Replay view should open immediately
   *  (consume-once: the panel clears it after focusing). Lets other panels
  ...
  /** When true, outgoing prompts are auto-enhanced via the AI prompt
   *  enhancer before being sent to the agent. Persisted under
   *  `cortex.enhancePrompt`. */
...
export const useCortexStore = create<CortexState>((set, get) => ({
  ...
  switchThread: (id) => {
    ...
    set({
      ...
      // Per-session UI state must reset on every sessionId change so FocusChain
      // re-hydrates from the new session's disk file instead of leaking (and
      // then autosaving) the previous thread's chain into this one.
      focusChain: [],
      queuedMessages: [],
    });
  },
