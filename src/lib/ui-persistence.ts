...
function sanitize(raw: unknown): PersistedUIState | null {
  ...
  return {
    // Always boot with the activity panel CLOSED — restoring the last-open panel
    // (e.g. Brain) left an extra, mostly-empty column on every launch. The
    // worktree context is still restored; panels open on demand.
    activityTab: null,
...
function equalUIState(a: PersistedUIState, b: PersistedUIState): boolean {
  return (
    ...
    a.activeProjectRoot === b.activeProjectRoot
  );
}
...
export function attachUIStatePersistence(debounceMs = 300): () => void {
  ...
  const snapshot = (): PersistedUIState => {
    ...
    return {
      ...
      activeProjectRoot: s.activeProject?.root ?? null,
    };
  };
