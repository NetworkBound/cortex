...
export interface Team {
   ...
   * (slice 5+). Absent until a synthesized run produces a parseable verdict;
   * rendered as a pass/fail scorecard on the manager card.
   */
  verdict?: Verdict | null;
}

/** Synthesis verification verdict — the manager's structured judgment. */
export interface Verdict {
  pass: boolean;
  /** 0–100 confidence/quality score. */
  score: number;
  checks?: VerdictCheck[];
}
