...
/**
 * How to install each launcher runtime the catalog relies on, keyed by the
 * entry `command`. Shown inline when the preflight probe (`probeMcpRuntimes`)
 * reports the command missing from PATH — without this, adding the server
 * "works" and then fails opaquely at the first tool call.
 */
export const RUNTIME_INSTALL_HINTS: Record<
  string,
