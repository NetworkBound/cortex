  ...
  tabCmd("gateway", "gateway", "Jump to the Cortex Gateway models + capabilities panel", ["caps"]),
  {
    ...
    run: async (args, ctx) => {
      ...
      try {
        ...
        // v2: workflows can declare template inputs — collect them via the
        // pre-run form before dispatch. Cancel aborts silently.
        let inputs: Record<string, string> | undefined;
        if ((wf.inputs?.length ?? 0) > 0) {
        ...
        if (!run) {
          ctx.append(errorNote(`/workflow: run of '${name}' failed.`));
          return;
        }
        ctx.append(
          systemNote(
