...
/** One OpenAI-compatible per-token API provider's config state (Settings →
 *  Providers → "API providers"). Mirrors the Rust `OpenAiCompatProvider`
 *  serde struct. No secret crosses the bridge — only whether a key is
 *  currently stored. */
export interface OpenAiCompatProvider {
  id: string;
  label: string;
  base_url: string;
  /** Is a key currently stored in the vault under `<id>/api-key`? */
  key_set: boolean;
  /** The env var this provider's adapter also falls back to. */
  api_key_env: string;
}

/** List every OpenAI-compatible per-token provider with its current
...
// Per-provider "Sync history" toggle that pulls your web chat history
// (Claude.ai / ChatGPT) into Cortex and keeps it fresh on a schedule. The web
// session is captured by browser auto-detect first, with a one-time in-app
// login fallback (`historySyncConnect`). No session cookie/token ever crosses
// the bridge in a response. Mirrors `commands::history_sync` in src-tauri.
...
export interface ProviderSyncStatus {
  ...
  label: string;
  enabled: boolean;
  /** Epoch-millis of the last sync, or null. */
  last_sync: number | null;
  /** Conversations imported from this provider so far. */
  conversation_count: number;
  /** "browser" | "login" | null — where the working session came from. */
  session_source: string | null;
  /** True when no session is auto-detectable/stored → show "Connect". */
  needs_login: boolean;
}

/** Enable/disable automatic history sync for a provider. Enabling triggers an
 *  immediate sync plus the recurring background loop. */
export async function historySyncSetEnabled(
  provider: string,
  enabled: boolean,
): Promise<void> {
  return invoke("history_sync_set_enabled", { provider, enabled });
}

/** Current sync status for every web-history provider (Claude, ChatGPT). */
export async function historySyncStatus(): Promise<ProviderSyncStatus[]> {
  return invoke<ProviderSyncStatus[]>("history_sync_status");
}

/** Run a sync for one provider right now. Returns a short human summary. */
export async function historySyncNow(provider: string): Promise<string> {
  return invoke<string>("history_sync_now", { provider });
}

/** Login fallback: open a webview at the provider's MAIN app; once signed in,
 *  injected JS fetches every conversation through the provider's own API inside
 *  that authenticated webview and the JSON is imported. Resolves with a summary
 *  once the fetch+import completes (or rejects on close/timeout). Live progress
 *  arrives on the `history_sync:progress` event ({@link HISTORY_SYNC_PROGRESS_EVENT}). */
export async function historySyncConnect(provider: string): Promise<string> {
  return invoke<string>("history_sync_connect", { provider });
}

/** Event name carrying live "fetched N conversations" progress during a
 *  `historySyncConnect` webview fetch. Mirrors `webview_fetch::PROGRESS_EVENT`. */
export const HISTORY_SYNC_PROGRESS_EVENT = "history_sync:progress";

/** Payload of {@link HISTORY_SYNC_PROGRESS_EVENT}. No secrets — counts + phase. */
export interface HistorySyncProgress {
  ...
  fetched: number;
  /** Total conversations discovered (0 until known). */
  total: number;
...
// ── Embedded Tailscale ───────────────────────────────────────────────────────
// Cortex ships a userspace Tailscale node (Go sidecar on tailscale.com/tsnet)
// that joins your tailnet with no root/daemon/admin and exposes a local SOCKS5
// proxy. These wrappers mirror the `ts_*` Tauri commands. The serde tag is
// `state` (snake_case), matching `TsStatus` in src-tauri/src/tailscale/mod.rs.
// The auth key is write-only: it crosses the bridge into the OS keychain and
// is never returned — never log it.
export type TsStatus =
  | { state: "disconnected" }
  | { state: "needs_login"; url: string }
  | { state: "connected"; ip: string; dnsname: string }
  | { state: "error"; msg: string };

/** Enable embedded Tailscale (optionally storing a fresh auth key) and spawn
 *  the sidecar. Returns the *initial* status — poll {@link tsStatus} for
 *  transitions to `needs_login`/`connected`. */
export async function tsEnable(authkey?: string): Promise<TsStatus> {
  return invoke<TsStatus>("ts_enable", { authkey: authkey ?? null });
}

/** Disable embedded Tailscale: stop the sidecar and persist the off flag. */
export async function tsDisable(): Promise<void> {
  return invoke("ts_disable");
}

/** Snapshot the current embedded-Tailscale status (incl. login URL). */
export async function tsStatus(): Promise<TsStatus> {
  return invoke<TsStatus>("ts_status");
}
...
export interface MobilePairing {
  ...
  url: string;
  /** Inline SVG QR code encoding {@link url}. */
  qr_svg: string;
}

/** Reachable mobile URL + a QR to scan. Rejects (with guidance) until the
 *  tailnet node is connected and has a MagicDNS name. */
export async function tsMobilePairing(): Promise<MobilePairing> {
  return invoke<MobilePairing>("ts_mobile_pairing");
}
