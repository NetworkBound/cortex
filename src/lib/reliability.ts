...
/** One MCP tool's aggregate within the window (issue 008 full scope). */
...
/**
 * MCP tool-call activity across the window (issue 008 full scope): overall
 * count/latency plus a per-tool breakdown. `est_usd` is a coarse estimate —
 * the summed whole-run cost of every run that made at least one MCP call, not
 * a per-call figure (token usage isn't tracked per tool call).
 */
