...
export function GatewayCapabilitiesPanel() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (mounted) setActiveModel(cfg.model || null);
      } catch {
        /* leave activeModel null if config is unavailable */
      }
    })();
    void tick();
  ...
  async function selectModel(id: string) {
    try {
      ...
      try {
        const cfg = await getGatewayConfig();
        setActiveModel(cfg.model || null);
      } catch {
        /* keep optimistic value if the refresh read fails */
      }
