...
export function KnowledgeGraph() {
  ...
  const visibleIds = useMemo(() => {
    ...
    // `simRef` is populated by the layout effect AFTER this render, so we must
    // also recompute when `frame` bumps (sim built / resettled) — otherwise the
    // first pass runs with simRef===null, returns an empty set, and the whole
    // graph renders permanently dimmed.
  }, [query, graph, frame]);

  const sim = simRef.current;
  const isEmpty = !sim || sim.nodes.length === 0;

  // Edge rendering looks up both endpoints by id every frame; a linear
  // `nodes.find()` per endpoint makes that O(edges×nodes). Build an id→node
  // map once instead. Like `visibleIds`, key off `frame` so it rebuilds when
  // the layout effect repopulates `simRef` after this render.
  const nodeById = useMemo(() => {
    const m = new Map<string, SimNode>();
    const s = simRef.current;
    if (s) for (const n of s.nodes) m.set(n.id, n);
    return m;
  }, [graph, frame]);
