...
export function RoutinesPanel() {
  ...
  // Tells "no routines" (genuine empty) apart from "couldn't reach the
  ...
  return (
    <div className="routines-panel">
      ...
      {!loaded && routines.length === 0 ? (
        <PanelLoading label="Loading routines" />
      ) : loadError && routines.length === 0 ? (
        <div className="routines-hint routines-error" role="alert">
          Couldn't load routines. {loadError}{" "}
          <button className="routines-act" onClick={() => void reload()}>
            Retry
          </button>
        </div>
      ) : routines.length === 0 ? (
