...
export function ChatPane() {
  ...
  useEffect(() => {
    ...
    // app shell, which is programmatically scrollable despite overflow:hidden)
    // when injected content (a long path/URL/code on project/chat open) overflows
    // wide, sliding the whole nav off-screen.
  ...
  async function performSend(
    ...
    // Routing is handled backend-side: local CLIs, gateway, Ollama, etc.
    // are all tried in priority order. No frontend gate needed.
    ...
    let userMsg = await expandSnippets(rawText);
    const sendImages = imgs.map((img) => img.dataUrl);
    setSending(true);
    setLastRoutingReason(null);
    if (enhancePrompt && userMsg.trim() && !userMsg.startsWith("/")) {
      try {
        setEnhancing(true);
        const enhanced = await invoke<string>("enhance_prompt", {
          message: userMsg,
          context: activeProject?.root ?? null,
        });
        if (enhanced && enhanced.trim()) userMsg = enhanced;
      } catch {
    ...
    const userContent =
      imgs.length > 0
        ? `${userMsg}${userMsg ? "\n\n" : ""}${imgs
            ...
            .join("\n")}`
        : userMsg;
    // Snapshot history BEFORE appending this turn: chatSend takes the new turn
    // as `message` separately, so if `history` also included it the model would
    // see the user's message twice (and adapters concatenate both).
    ...
    if (compareOn && compareModels.length >= 2) {
      if (imgs.length > 0) {
        // Multi-model compare goes through arenaSend, which is text-only — be
        // explicit rather than silently dropping the attachments.
        pushToast({
          title: "Compare ignores images",
          body: "Multi-model compare sends text only — attachments were dropped for this turn.",
          kind: "warning",
        });
      }
      try {
  ...
  async function handleFilePick(files: FileList) {
    const imageResult = await extractImageAttachments(files, images);
    if (imageResult.attachments.length > 0) {
      setImages((cur) => [...cur, ...imageResult.attachments]);
    }
    setImageSkipped(imageResult.skipped);
