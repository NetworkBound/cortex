...
function SkillRunner({ skill, onAppend }: SkillRunnerProps) {
  ...
  useEffect(() => {
    ...
    setVars(seed);
    setStatus(null);
    // Key on the skill identity + a stable signature of its input names rather
    // than the `inputs` array reference — a manual refresh produces a fresh
    // array each time, which would otherwise wipe the user's in-progress entries.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [skill.name, skill.inputs.map((i) => i.name).join(",")]);
