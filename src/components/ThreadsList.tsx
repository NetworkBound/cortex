  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (disk.length > 0) {
        ...
        // No saved threads for this project. Start clean: blank the active
        // session and keep ONLY it — otherwise the previous project's threads
        // linger in the sidebar and the autosave loop below cross-saves them
        // under THIS project's root.
        resetSession();
        const active = useCortexStore.getState().getActiveThread();
        if (active) setThreads([active]);
      }
    })();
    return () => {
      cancelled = true;
    };
  }, [projectRoot, setThreads, resetSession]);
