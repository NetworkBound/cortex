...
export function MultiBuffer() {
  ...
  function onChange(id: string, body: string) {
    ...
    if (current.dirty !== isDirty) patchExcerpt(id, { dirty: isDirty });
  }
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      // CodeMirror's cell keymap already handles Mod-S (and preventsDefault)
      // when an editor has focus — skip those so we don't double-save.
