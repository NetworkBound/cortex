...
export function GitHistoryPanel() {
  ...
  // Mirror of `commits.length` for the polling interval to read without
  // re-subscribing on every load (avoids tearing down the 15s timer).
  const loadedCountRef = useRef(0);

  const root = project?.root ?? null;

  // Keep the loaded-count mirror in sync so the polling interval refreshes
  // the right depth without depending on `commits` (which would reset the timer).
  useEffect(() => {
    loadedCountRef.current = commits.length;
  }, [commits]);
  ...
  useEffect(() => {
    if (!root) return;
    let cancelled = false;
    // Refresh re-fetches *all* currently-loaded commits in one call and
    // replaces the list wholesale. Merging page 0 against a numeric offset
    // corrupts the paging order when new commits land (the offset shifts),
    // so we re-read `max(loaded, PAGE_SIZE)` from the top instead.
    const load = async () => {
      setLoading(true);
      setError(null);
      try {
        const count = Math.max(loadedCountRef.current, PAGE_SIZE);
        const rows = await gitHistory(root, count, 0);
        if (cancelled) return;
        setCommits(rows);
        if (rows.length < count) setAtEnd(true);
      } catch (e) {
