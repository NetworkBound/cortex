...
interface UnifiedRow {
  ...
  modified: number;
  path: string;
  /** Backend relevance score for search hits (0 for recents). */
  score: number;
}
...
export function MemoryExplorer({ autoFocus = true }: { autoFocus?: boolean } = {}) {
  ...
  const rows: UnifiedRow[] = useMemo(() => {
    if (debouncedQuery) {
      ...
      const cRows: UnifiedRow[] = chatHits.map((h) => ({
        ...
        path: h.file_path,
        // Chat search has no backend relevance score; rank these by recency
        // (the modified tiebreaker) among themselves.
        score: 0,
      }));
      // With a query active, rank by backend relevance (score) first so the
      // most relevant hit wins regardless of kind; fall back to recency.
      return [...mRows, ...cRows].sort(
        (a, b) => b.score - a.score || b.modified - a.modified,
      );
    }
  ...
  const filtered = useMemo(() => {
    // Search hits are mapped sourceKind:"memory" (the backend hit carries a
    // human label, not the source_kind enum), so a sub-source filter like
    // "obsidian"/"runbooks" would hide them. Results are already query-scoped,
    // so bypass the source filter whenever a query is active.
    if (debouncedQuery) return rows;
    return rows.filter((r) => sourceMatchesFilter(r, filter));
