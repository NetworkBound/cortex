...
function ManagerCard({ team }: { team: Team }) {
  return (
    <div className="orch-manager-card">
      ...
      {team.verdict ? <VerdictCard verdict={team.verdict} /> : null}
...
/**
 * Pass/fail scorecard for a run's synthesis verdict (slice 5+). Answers "did
 * it work?" at a glance — a colored header (pass/fail + score) over the
 * per-check line items the manager reported — instead of burying it in the
 * synthesis prose. Only rendered when a verdict was parsed.
 */
function VerdictCard({ verdict }: { verdict: NonNullable<Team["verdict"]> }) {
  const checks = verdict.checks ?? [];
  return (
    <div className={`orch-verdict ${verdict.pass ? "pass" : "fail"}`}>
      <div className="orch-verdict-head">
