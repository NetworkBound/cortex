 ...
 * seeds ten sensible defaults on first launch: the original five
 * (review-pr, morning-standup, triage-bug, prep-release, audit-deps) plus
 * the five-recipe v2 catalog (audit-repo, fix-failing-tests, release-notes,
 * pr-review, security-scan-before-merge) — each of the latter declares
 * inputs and pins at least one step's model.
 *
 ...
 *
 * v2 (strictly additive — v1 workflows behave exactly as before): a
 * workflow may declare `inputs` (collected via a pre-run form and expanded
 * server-side as exact-key `{{key}}` placeholders), each step may pin a
 * `model` (picked from the same source as the composer's ModelPicker and
 * routed through the existing chat model override), and `pipe_output`
 * marks a step whose answer prefixes the next step's prompt at dispatch.
 *
 * Full scope also adds Import / Export: "Export" asks the backend for the
 * workflow's canonical YAML and writes it to a user-chosen path via the
 * native save dialog; "Import" reads a user-chosen `.yaml` file back into a
 * new workflow (refusing to clobber an existing name). Both round-trip
 * through the same YAML shape `save_workflow` persists, so a v1 file stays
 * byte-identical across an export/import cycle.
 *
...
interface Draft {
  ...
  name: string;
  description: string;
  /** v2 declared inputs — pass-through only (edited via YAML for now), but
   *  MUST survive a panel save or an edit would silently drop them. */
...
export function WorkflowsPanel() {
  ...
  /** Model universe for the per-step model picker (same source as the
   *  composer's ModelPicker). Best-effort: empty list = "Auto" only. */
  const [models, setModels] = useState<ModelEntry[]>([]);
  /** Full scope: import/export busy flags — `busyExport` keyed by workflow
   *  name so only that card's button shows the in-flight state. */
  ...
  // Fetch the model list on mount and refresh on backend `models:changed`
  // events, mirroring ModelPicker — a freshly pulled Ollama tag should show
  // up in the step editor without a restart.
  useEffect(() => {
    let alive = true;
    const refresh = () => {
      listModels()
  ...
  async function handleSave() {
    ...
    if (trimmedName !== draft.origName) {
      const existingNames = new Set((workflows ?? []).map((w) => w.name));
      if (existingNames.has(trimmedName)) {
        pushToast({
          title: "Name already exists",
          body: `A workflow named "${trimmedName}" already exists.`,
          kind: "warning",
        });
        return;
      }
    ...
    try {
      const saved = await saveWorkflow({
        ...
        // Pass declared inputs through untouched — the panel doesn't edit
        // them yet, but dropping them on save would break a v2 workflow.
        inputs: draft.inputs,
        steps: cleanedSteps,
      });
      if (!saved) {
  ...
  /**
   * Launch a workflow. v2 workflows with declared inputs first collect
   * values via the pre-run form (sequential in-app prompts; cancel aborts);
   * the backend validates required keys and expands `{{key}}` server-side.
   * We then resolve the steps backend-side (single roundtrip, proper
   * path-safety enforcement) and append one system note per step so the
   * user can read the queue, copy a prompt into the composer, and let the
   * existing chat pipeline take it from there.
   */
  async function handleRun(name: string) {
    const wf = workflows?.find((w) => w.name === name);
    let inputs: Record<string, string> | undefined;
    if (wf && (wf.inputs?.length ?? 0) > 0) {
      const collected = await collectWorkflowInputs(wf);
      if (collected === null) {
        pushToast({ title: "Run cancelled", body: name, kind: "info" });
        return;
      }
      inputs = collected;
    }
  ...
  /**
   * Export a workflow: fetch its canonical YAML from the backend, then ask
   * the user where to save it via the native save dialog. A cancelled
   * dialog (`path` null) is a silent no-op, matching the cancel-aborts
   * convention `collectWorkflowInputs` already uses for Run.
   */
  async function handleExport(name: string) {
    setBusyExport(name);
    try {
      const yaml = await exportWorkflowYaml(name);
  ...
  /**
   * Import a workflow: let the user pick a `.yaml`/`.yml` file via the
   * native open dialog, then hand its path to the backend (which reads,
   * parses, and — refusing to clobber an existing name — persists it under
   * `~/.cortex/workflows/`). A cancelled dialog is a silent no-op.
   */
  async function handleImport() {
    ...
    try {
      ...
      const imported = await importWorkflowFromPath(selected);
      pushToast({ title: "Imported", body: imported.name, kind: "success" });
      await reload();
          ...
          >
            ...
            <div className="workflows-card-actions">
              ...
              <button
                type="button"
                className="link-btn"
                disabled={busyExport === wf.name}
                onClick={() => void handleExport(wf.name)}
                title="Export this workflow as a .yaml file"
              >
                {busyExport === wf.name ? "…" : "Export"}
              </button>
              <button
                type="button"
                  ...
                  <div key={i} className="workflows-step">
                    ...
                    <div className="workflows-step-opts">
                      <select
                        ...
                        title="Model for this step (Auto = default routing)"
                        value={step.model ?? ""}
                        onChange={(e) =>
                          updateStep(i, { model: e.target.value || null })
                        }
                      >
                      ...
                      <label
                        ...
                        title="Prefix the next step's prompt with this step's answer"
                      >
                        <input
                          type="checkbox"
                          checked={!!step.pipe_output}
                          onChange={(e) =>
                            updateStep(i, { pipe_output: e.target.checked })
                          }
                        />
                        pipe output → next step
                      </label>
                    </div>
                  </div>
                ))}
