...
import { loadSessionMessages, exportSessionToVault } from "@/lib/sessions";
...
export function SessionPicker() {
  ...
  /** The highlighted row, so arrow-key nav can scroll it into view. */
  ...
  async function pick(s: RecentSession) {
    ...
    try {
      ...
      // An empty/orphaned session has nothing to load; resuming it would wipe
      // the current chat for no gain. Surface it and bail without resuming.
      if (msgs.length === 0) {
        setPickError("This session has no messages to resume.");
        pushToast({ title: "Nothing to resume", kind: "info" });
        return;
      }
   ...
   *  vault as Markdown. Stops row-click propagation so it doesn't also resume. */
  async function saveToBrain(s: RecentSession) {
    try {
      const res = await exportSessionToVault(s.session_id);
      pushToast({
        title: "Saved to Brain",
        body: `${res.message_count} messages → ${res.written_path}`,
        kind: "success",
      });
    } catch (err) {
    ...
    <div className="palette-backdrop" onClick={() => setOpen(false)}>
      <div className="palette session-picker" onClick={(e) => e.stopPropagation()}>
        ...
        <ul>
          ...
          {filtered.map((s, i) => {
            const importTag = s.agents.find((a) => a.startsWith("import:"));
            const importLabel =
              importTag === "import:claude.ai"
                ? "Claude.ai"
                : importTag === "import:chatgpt"
                  ? "ChatGPT"
                  : importTag?.slice("import:".length);
            return (
              <li
                key={s.session_id}
              ...
              >
                ...
                <div className="sp-meta muted">
                  ...
                  <button
                    type="button"
                    className="sp-save"
                    title="Save this chat to your Obsidian vault as Markdown"
                    onClick={(e) => {
                      e.stopPropagation();
                      void saveToBrain(s);
                    }}
