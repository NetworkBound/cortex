...
export function RecipeGallery({ onClose }: RecipeGalleryProps) {
  ...
  const handleRun = (r: Recipe) => {
    // No dedicated recipe runtime yet — splice the recipe's goal into the
    // chat composer (the live `cortex:composer-insert` path ChatPane listens
    // for) so the user can review and fire it manually. Same UX as
        ...
        {editing ? (
          ...
          <div className="recipe-gallery-body">
            ...
            {!loaded && recipes.length === 0 ? (
              <PanelLoading label="Loading recipes" />
            ) : recipes.length === 0 ? (
